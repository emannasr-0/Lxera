...
Route::get('/save-data-in-session', function () {
    session()->put('main_category_id', 615);
    session()->put('sub_category_id', 616);
    session()->put('bundle_id', 32);
    session()->put('webinar_id', 2066);
});

Route::get('/preparation-courses', function () {
    // Serve the index.html file located in public/courses
    return response()->file(public_path('courses/index.html'));
});

Route::get('/get-language', function (Request $request) {
    return response()->json(['locale' => Session::get('locale', config('app.locale'))]);
});
...
Route::group(['prefix' => 'my_api', 'namespace' => 'Api\Panel', 'middleware' => 'signed', 'as' => 'my_api.web.'], function () {
    ...
    Route::get('/charge/{user}', 'PaymentController@webChargeRender')->name('charge');
...
Route::group(['namespace' => 'Auth', 'middleware' => ['check_mobile_app', 'share', 'check_maintenance']], function () {
    ...
    Route::get('/google', 'SocialiteController@redirectToGoogle')->name('login.google');
