...
class Webinar extends Model implements TranslatableContract
{
    ...
    public $translatedAttributes = ['title', 'description', 'seo_description', 'requirements'];
    ...
    {
        return getTranslateAttributeValue($this, 'requirements');
    }
    ...
    {
        return $this->belongsToMany(User::class, 'webinar_partner_teacher', 'webinar_id', 'teacher_id');
    }
    ...
    public function checkUserHasBought($user = null, $checkExpired = true, $test = false): bool
    {
        ...
        if (!empty($user)) {
            ...
            if (!empty($sale)) {
                ...
                if ($hasBought && !empty($this->access_days) && $checkExpired) {
                    $hasBought = $this->checkHasExpiredAccessDays($sale->created_at, $sale->gift_id);
                }
            }

            if (!$hasBought) {
                $hasBought = ($this->creator_id == $user->id || $this->teacher_id == $user->id);
            ...
            if (!$hasBought) {
                ...
                if ($bundleWebinar->isNotEmpty()) {
                    foreach ($bundleWebinar as $item) {
                        if (!empty($item->bundle) && $item->bundle->checkUserHasBought($user)) {
                            $hasBought = true;
                        }
                    }
                }
    ...
    public function getProgressApi($isLearningPage = false, $userId = null)
    {
        $progress = 0;

        // If no user ID passed, use the authenticated user
        $user_id = $userId ?? auth()->id();

        if (
            $user_id &&
            $this->checkUserHasBought($user_id) && // we will also make this accept user id
            (
                !$this->isWebinar() ||
                ($this->isWebinar() && $this->isProgressing()) ||
                $isLearningPage
            )
        ) {
    ...
    function isUserHasAccessToContent()
    {
        ...
        foreach ($this->groups as $group) {
            ...
            if ($group->status == 'active' && $startDate <= $now) {
