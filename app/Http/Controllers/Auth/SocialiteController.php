...
class SocialiteController extends Controller
{
        ...
        try {
            ...
            if (empty($user)) {
                $user = User::create([
                    ...
                    'role_id' => 13,
                    'role_name' => Role::$registered_user,
                    'status' => User::$active,
                    'verified' => true,
                    'created_at' => time(),
                    'password' => null
    ...
    public function handleFacebookCallback(Request $request)
    {
      ...
      // LinkedIn Redirect and Callback
      ...
      public function handleLinkedInCallback(Request $request) 
      {
          ...
          try {
              $account = Socialite::driver('linkedin')->user();
  
              // Try to find the user in the database using LinkedIn ID or email
              $user = User::where('linkedin_id', $account->id)
                  ->orWhere('email', $account->email)
                  ->first();
              ...
              // Update the user's LinkedIn ID
              $user->update(['linkedin_id' => $account->id]);
              ...
              [
                'title' => trans('public.request_failed'),
               'msg' => trans('auth.fail_login_by_linkedin'), 
               'status' => 'error'
               ]]);
          }
      }
      ...
      public function handleAppleCallback(Request $request)
      {
          try {
              ...
              $account = Socialite::driver('apple')->user();
  
              // Find or create the user based on Apple data
              $user = User::where('apple_id', $account->getId())
              ...
              $toastData = [
                  'title' => trans('public.request_failed'),
                  'msg' => trans('auth.fail_login_by_apple'),
                  'status' => 'error'
              ];
