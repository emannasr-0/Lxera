...
// dd($carts);
    ...
    private function handleDiscountPrice($discount, $carts, $subTotal)
    {
        ...
        elseif ($discount->source == Discount::$discountSourceBundleinstallment) {
            $totalInstallmentBundlesAmount = 0;
            $installmentOtherDiscounts = 0;
            $discountBundlesIds = $discount->discountBundles()->pluck('bundle_id')->toArray();
    
            foreach ($carts as $cart) {
                $bundle = $cart->bundle;
                if (!empty($bundle) and in_array($bundle->id, $discountBundlesIds)) {
                    // Check if the bundle is paid in installments
                    if (!empty($cart->installmentPayment)) {
                        // Assuming each installment has a price associated with it
                        // You might need to adapt this based on how you store installment data in your system
                        $totalInstallmentBundlesAmount += $cart->installmentPayment->installment_price;  // Adjust based on your data structure
                    } else {
                        // If not an installment bundle, use the full price
                        $totalInstallmentBundlesAmount += $bundle->price;
                    }
                }
            }
    
            if ($discount->discount_type == Discount::$discountTypeFixedAmount) {
                $totalDiscount = ($totalInstallmentBundlesAmount > $discount->amount) ? $discount->amount : $totalInstallmentBundlesAmount;
            } else {
                $totalDiscount = ($totalInstallmentBundlesAmount > 0) ? $totalInstallmentBundlesAmount * $percent / 100 : 0;
            }
        }
