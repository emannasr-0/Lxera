...
class CartManagerController extends Controller
{
            ...
            if (!empty($cookieCarts)) {
                ...
                if (!empty($cookieCarts) and count($cookieCarts)) {
                    ...
                    foreach ($cookieCarts as $cookieCart) {
                        ...
                        if (!empty($cookieCart['item_name']) and $cookieCart['item_name'] == 'webinar_id') {
                        ...
                        }elseif (!empty($cookieCart['item_name']) and $cookieCart['item_name'] == 'certificate_template_id') {
                            $certificate_template = CertificateTemplate::where('id', $cookieCart['item_id'])
                                ->where('status', 'active')
                                ->first();
                            ...
                            if (!empty($certificate_template)) {
                                ...
                                $item = new Cart();
                                $item->certificate_template_id = $certificate_template->id;
                                $item->certificate_template = $certificate_template;
                                $item->ticket = $ticket;
                                $item->ticket_id = !empty($ticket) ? $ticket->id : null;
                    ...
                    if (!empty($carts)) {
                        foreach ($carts as $cart) {
                            if (!empty($cart['item_name']) and !empty($cart['item_id'])) {
                                ...
                                } elseif ($cart['item_name'] == 'certificate_template_id') {
                                    $this->storeUserCertificateCart($user, $cart);
                                }
                            }
                        }
                    }
    ...
    public function storeUserForm_feeCart($user, $data)
    {
        $ticket_id = $data['ticket_id'] ?? null;
    ...
    public function storeUserCertificateCart($user, $data)
    {
        $certificate_id = $data['item_id'];
        $certificate_bundle_id=$data['certificate_bundle_id'];
        $ticket_id = $data['ticket_id'] ?? null;

        $certificate = CertificateTemplate::where('id', $certificate_id)->first();

        if (!empty($certificate) and !empty($user)) {

            Cart::updateOrCreate([
                'creator_id' => $user->id,
                'certificate_template_id' => $certificate_id,
                'certificate_bundle_id'=>$certificate_bundle_id
            ], [
                'ticket_id' => $ticket_id,
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            'item_id' => 'required',
            'item_name' => 'nullable',
            'certificate_bundle_id' => 'nullable',
        ]);

        $data = $request->except('_token');
        $item_name = $data['item_name'];
        $certificate_bundle_id=$data['certificate_bundle_id'] ?? null;
        ...
        if (!empty($user)) { // store in DB
            ...
            elseif ($item_name == 'certificate_template_id') {
                $result = $this->storeUserCertificateCart($user, $data);
            }
            // elseif ($item_name == 'form_fee') {
            //     $result = $this->storeUserForm_feeCart($user, $data);
            // }
