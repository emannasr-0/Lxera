...
class JobsController extends Controller
{
    ...
    //                 '[time.date]' => dateTimeFormat($dueAt, 'j M Y '),
    ...
    public function sendInstallmentReminders()
    {
        ...
        foreach ($steps as $step) {
            ...
            if (empty($checkPayment)) {
                ...
                // Send "before_due" reminder if within the reminder window
                if (!empty($reminderBeforeOverdueDays) and $daysLeft > 0 and $daysLeft < $reminderBeforeOverdueDays) {
                    $template = "reminder_installments_before_overdue";
                    $reminderType = "before_due";
    
                    // Check if the "before_due" reminder has already been sent
                    $checkReminder = InstallmentReminder::query()->where('installment_order_id', $order->id)
                        ->where('installment_step_id', $step->id)
                        ->where('user_id', $order->user_id)
                ...
                // Send "after_due" reminder if the installment is overdue
                else if ($daysLeft < 0) {
                    $template = "installment_due_reminder";
                    $reminderType = "due";
    
                    // Check if the "after_due" reminder has already been sent, or if it's time to send the next one
                    $checkReminder = InstallmentReminder::query()->where('installment_order_id', $order->id)
                        ->where('installment_step_id', $step->id)
                        ->where('user_id', $order->user_id)
                        ->where('type', 'after_due')
                        ->first();
    
                    $lastReminder = $checkReminder ? $checkReminder->created_at : 0;
                   $reminderInterval = 7 * 86400; // 1 week in seconds
                   // $reminderInterval = 60; 
                    // If no reminder has been sent or it's been a week since the last one, send the "after_due" reminder
                    if (empty($checkReminder) || ($timestamp - $lastReminder >= $reminderInterval)) {
                        // Update the reminder or create a new one
                        if ($checkReminder) {
                            $checkReminder->update(['created_at' => $timestamp]);
                        } else {
                            InstallmentReminder::query()->create([
                                'installment_order_id' => $order->id,
                                'installment_step_id' => $step->id,
