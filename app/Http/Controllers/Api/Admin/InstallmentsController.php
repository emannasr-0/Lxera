...
class InstallmentsController extends Controller
{
    ...
    public function index()
    {
        ...
        $groupUser = Group::where('status', 'active')->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $installment = Installment::create([
                ...
                'end_date' => $endDate,
                'verification' => $data['verification'] ?? 'off',
                'request_uploads' => $data['request_uploads'] ?? 'off',
                'bypass_verification_for_verified_users' => $data['bypass_verification_for_verified_users'] ?? 'off',
            ...
            if ($installment) {
                ...
                $installment->load([
                    'steps.translations',
                    'userGroups',      // مهم لجلب group_ids
                    'specificationItems' // مهم لجلب specificationItems
 
                ]);
                $installment->group_ids = $installment->userGroups()->pluck('group_id');
 
                // Append specification items
                $installment->specificationItems = $installment->specificationItems()->get();
                return response()->json([
            ...
            foreach ($data['steps'] as $stepId => $stepData) {
                ...
                if ($step) {
                    $step->update([
                        ...
                        'order'        => $order,
                        'title'          => $stepData['title'] ?? null,   // أضف هذا
                    ]);
                } else {
