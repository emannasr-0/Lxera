...
class CertificatesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $studentNameFilter = $request->input('student_name');

        $query = Certificate::whereNull('webinar_id');

        if (!empty($filters['title'])) {
            $query->whereHas('quiz.translations', function ($query) use ($filters) {
                $query->where('title', 'like', '%' . $filters['title'] . '%');
            });
        }

        if (!empty($teacherNameFilter)) {
            $query->whereHas('quiz.teacher', function ($query) use ($teacherNameFilter) {
                $query->filterBySearch(['full_name' => $teacherNameFilter]);
            });
        }

        if (!empty($studentNameFilter)) {
            $query->whereHas('student', function ($query) use ($studentNameFilter) {
                $query->filterBySearch(['full_name' => $studentNameFilter]);
            });
        }
    ...
    public function CertificatesTemplatesList(Request $request)
    {
        ...
        $templates = CertificateTemplate::orderBy('created_at', 'desc')->with(['bundle', 'webinar'])
            ->paginate(10);
   ...
   public function CertificatesTemplateStore($url_name, Request $request)
{
    ...
    public function CertificatesTemplateUpdate($url_name, Request $request, $template_id)
    {
        try {
            ...
            $request->validate([
                'title' => 'sometimes|required_with:locale|string',
                'image' => 'sometimes|image|mimes:jpeg,png,jpg|max:2048',
                'type' => 'sometimes|in:quiz,course,bundle,attendance,new_verssion_bundle,new_verssion_course,new_verssion_attendance',

                'position_x_student' => 'sometimes|numeric',
                'position_y_student' => 'sometimes|numeric',
                'font_size_student' => 'sometimes|numeric',

                'position_x_course' => 'sometimes|numeric',
                'position_y_course' => 'sometimes|numeric',
                'font_size_course' => 'sometimes|numeric',

                'position_x_date' => 'sometimes|numeric',
                'position_y_date' => 'sometimes|numeric',
                'font_size_date' => 'sometimes|numeric',

                'position_x_gpa' => 'sometimes|numeric',
                'position_y_gpa' => 'sometimes|numeric',
                'font_size_gpa' => 'sometimes|numeric',

                'position_x_certificate_code' => 'sometimes|numeric',
                'position_y_certificate_code' => 'sometimes|numeric',
                'font_size_certificate_code' => 'sometimes|numeric',

                'text_color' => 'sometimes|string',
                'locale' => 'sometimes|required_with:title|string',

                'bundles' => 'sometimes|array',
                'bundles.*' => 'exists:bundles,id',
                'webinars' => 'sometimes|array',
                'webinars.*' => 'exists:webinars,id',
            ]);
            ...
            if ($request->filled(['title', 'locale'])) {
