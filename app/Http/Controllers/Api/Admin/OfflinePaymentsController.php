...
class OfflinePaymentsController extends Controller
{
    public function index($url_name, Request $request)
    {
        ...
        $offlinePayments->getCollection()->transform(function ($offlinePayment) {
            $offlinePayment->pay_for_label = $this->getPayForLabel($offlinePayment);
            return $offlinePayment;
        });

        foreach ($offlinePayments as $offlinePayment) {
            $offlinePayment->bank_name = $offlinePayment->offlineBank->title;
        }
        $data = [
            'offlinePayments' => [
                'current_page' => $offlinePayments->currentPage(),
                'last_page' => $offlinePayments->lastPage(),
                'per_page' => $offlinePayments->perPage(),
                'total' => $offlinePayments->total(),
                'data' => $offlinePayments->items(),
            ],
    ...
    private function getPayForLabel($offlinePayment)
    {
        if ($offlinePayment->pay_for == 'form_fee') {
            return 'رسوم حجز مقعد دراسي';
        } elseif ($offlinePayment->pay_for == 'bundle') {
            $title = optional($offlinePayment->order->orderItems->first()->bundle)->title;
            return $title ? 'الدفع كامل ل ' . $title : 'الدفع كامل للبندل';
        } elseif ($offlinePayment->pay_for == 'webinar') {
            $title = optional($offlinePayment->order->orderItems->first()->webinar)->title;
            return $title ? 'الدفع كامل ل ' . $title : 'الدفع كامل للدورة';
        } elseif ($offlinePayment->pay_for == 'installment') {
            $stepTitle = optional(optional($offlinePayment->order->orderItems->first()->installmentPayment)->step->installmentStep)->title ?? 'القسط الأول';
            $bundleTitle = optional($offlinePayment->order->orderItems->first()->bundle)->title;
            return $stepTitle . ' ل ' . $bundleTitle;
        } elseif ($offlinePayment->pay_for == 'service') {
            $serviceTitle = optional($offlinePayment->order->orderItems->first()->service)->title;
            return $serviceTitle ? 'رسوم طلب خدمة ' . $serviceTitle : 'رسوم طلب خدمة';
        }
        return '';
    }
