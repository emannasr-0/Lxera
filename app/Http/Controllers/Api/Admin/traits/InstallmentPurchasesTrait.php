...
trait InstallmentPurchasesTrait
{
    public function purchases()
    {
        ...
        $handleSafePrice = function ($price) {
            $numericPrice = preg_replace('/[^0-9.\-]/', '', (string)$price);
            $numericPrice = (float) $numericPrice;
            return round($numericPrice, 2);
        };

        $orders->getCollection()->transform(function ($order) use ($handleSafePrice) {

            $installments = $order->selectedInstallment->steps ?? [];
            ...
            return [
                ...
                'totalAmount' => $handleSafePrice($order->getCompletePrice()),
                'firstInstallment' => $order->selectedInstallment->upfront,
                'InstallmentsCount' => $order->selectedInstallment->steps_count,
                'InstallmentsTotalAmount' => $installmentsTotal,
                'InstallmentsAmount' => $order->selectedInstallment->steps,
                'lateInstallments' => $order->overdue_count,
                'overdueAmount' => $handleSafePrice($order->overdue_amount),
                'FirstOverdueInstDate' => $order->upcoming_date,
                'remainingDays' => $order->days_left,
                'status' => $order->status,
            ];
        });

        return response()->json($orders, 200, [], JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
    }
            ...
            $order->upcoming_date = !empty($order->bundle)
                ? (!empty($getUpcomingInstallment)
                    ? Carbon::createFromTimestamp(
                        ($order->selectedInstallment->deadline_type == 'days')
                            ? (($getUpcomingInstallment->deadline * 86400) + $order->bundle->start_date)
                            : $getUpcomingInstallment->deadline,
                        'Asia/Riyadh'
                    )
                    ->toDateString()
                    : ''
