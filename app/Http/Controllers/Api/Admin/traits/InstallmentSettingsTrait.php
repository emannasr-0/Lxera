...
trait InstallmentSettingsTrait
{
    public function settings(Request $request)
    {
        ...
        // فك JSON الموجود في value
        foreach ($settings as $setting) {
            foreach ($setting->translations as $translation) {
                if (!empty($translation->value) && $this->isJson($translation->value)) {
                    $translation->value = json_decode($translation->value, true);
                }
            }
        }
    ...
    public function storeSettings(Request $request)
    {
        ...
        if (!empty($newValues) && !empty($values)) {
