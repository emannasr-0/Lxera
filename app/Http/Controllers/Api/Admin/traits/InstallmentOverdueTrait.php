...
trait InstallmentOverdueTrait
{
    public function overdueLists(Request $request)
    {
        ...
        $orders = $this->getInstallmentFilter($query, $request)->with(['installment', 'bundle', 'user'])->paginate(10);
    ...
    public function overdueHistories(Request $request)
    {
        ...
        $orders = $this->getInstallmentFilter($query, $request)
            ->with(['installment', 'bundle', 'user'])
        ->paginate(20);


        // تعديل كل عنصر مباشرة
        $orders->each(function ($order) {
            $paidTimestamp = $order->status === 'paid' ? $order->paid_at : time();

            // أضف عدد الأيام المتأخرة
            $order->overdue_days = round(($paidTimestamp - $order->overdue_date) / 86400);

            // تعديل paid_at وتنسيقه بنفس الشكل الموجود في الblade
            $order->paid_at_formatted = $order->status === 'paid'
                ? date('j M Y', $order->paid_at)
                : '-';
        });
