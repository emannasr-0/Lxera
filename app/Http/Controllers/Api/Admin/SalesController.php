...
class SalesController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($sales as $sale) {
            ...
            $sale->class_title = $sale->class->title;
    ...
    private function getSaleTypeLabel($sale)
    {
        if ($sale->type == \App\Models\Sale::$registrationPackage) {
            ...
            return trans('update.product');
        } elseif ($sale->type == \App\Models\Sale::$bundle) {
            return ($sale->payment_method == 'scholarship') ? 'منحة دراسية' : 'دفع كامل الرسوم';
        } elseif ($sale->type == \App\Models\Sale::$gift) {
            return trans('update.gift');
        } elseif ($sale->type == \App\Models\Sale::$installmentPayment) {
            return $sale->order->orderItems->first()->installmentPayment->step->installmentStep->title ?? 'قسط التسجيل';
        } elseif ($sale->type == 'form_fee') {
            return 'رسوم حجز مقعد';
        } elseif ($sale->type == 'certificate') {
            return 'شراء شهادة';
        } elseif ($sale->type == 'webinar') {
            return 'دورة';
        } elseif ($sale->type == 'service') {
            return 'خدمة الكترونية';
        }
    ...
    {
        $typeOptions = [];
        $types = ['webinar', 'subscribe', 'promotion', 'registration_package', 'product', 'bundle', 'installment_payment', 'gift', 'certificate', 'form_fee', 'service', 'transform_bundle', 'bridging'];
