...
class WebinarCertificateController extends Controller
{
    public function index(Request $request)
    {
        ...
        $teacherNameFilter = $request->input(['teacherName']);
        $webinarTitleFilter = $request->input('webinar_title');
        $bundleTitleFilter = $request->input('bundle_title');
        ...
        $certificates = $query->with([
            ...
            ->whereHas('webinar.translation', function ($query) use ($webinarTitleFilter) {
                $query->where(function ($q) use ($webinarTitleFilter) {
                    if ($webinarTitleFilter) {
                        $q->where('title', 'like', "%$webinarTitleFilter%");
                    }
                });
            })
            ->whereHas('bundle.translation', function ($query) use ($bundleTitleFilter) {
                $query->where(function ($q) use ($bundleTitleFilter) {
                    if ($bundleTitleFilter) {
                        $q->where('title', 'like', "%$bundleTitleFilter%");
                    }
                });
            })
            ->whereHas('webinar.teacher', function ($query) use ($teacherNameFilter) {
