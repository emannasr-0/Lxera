...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $this->validate($request, [
                ...
                'full_name' => 'required|min:3|max:128|string',
    ...
    public function update($url_name, Request $request, $id)
    {
        try {
            ...
            $user->fill([
                ...
                'about' => $data['about'] ? $user->about : null,
    ...
    public function Courses($url_name, Request $request, $id, $is_export_excel = false)
    {
        $this->authorize('admin_users_list');
        ...
        $groups = $webinar->groups->unique('id');

        $totalGroups = $groups->count();

        $filteredGroups = $this->filters($groups, $request);

        $mappedGroups = $filteredGroups->map(function ($group) {
            $group->enrollments_count = $group->enrollments()->count();
            return $group;
        });
        ...
        $paginated = new LengthAwarePaginator(
            $mappedGroups->forPage($page, $perPage)->values(),
            $mappedGroups->count(),
            $perPage,
            $page
        );
        ...
        if ($is_export_excel) {
            return $mappedGroups->sortByDesc('created_at')->values();
        }
    ...
    public function groupInfo($url_name, $group_id)
    {
        $organization = Organization::where('url_name', $url_name)->first();
        if (!$organization) {
            return response()->json(['message' => 'Organization not found'], 404);
        ...
        $group = Group::with(['enrollments.user'])->find($group_id);
        if (!$group) {
        ...
        $enrollmentsCollection = $group->enrollments;

        $paginatedEnrollments = new LengthAwarePaginator(
            $enrollmentsCollection->forPage($page, $perPage)->values(),
            $enrollmentsCollection->count(),
            $perPage,
            $page,
        ...
        return response()->json([
            'pageTitle' => trans('public.students'),
            'totalStudents' => $enrollmentsCollection->count(),
            'enrollments' => $paginatedEnrollments,
        ]);
    }


    public function groupUpdate($url_name, Request $request, Group $group)
    {
        try {
            $organization = Organization::where('url_name', $url_name)->first();
            if (!$organization) {
            ...
            return response()->json($toastData, 200);
        } catch (\Illuminate\Validation\ValidationException $e) {
            $formattedErrors = [];
    ...
    function changeGroup(Request $request, $url_name)
    {
        try {
            $organization = Organization::where('url_name', $url_name)->first();
            if (!$organization) {
    ...
    function groupExportExcel($url_name, Group $group)
    {
        $organization = Organization::where('url_name', $url_name)->first();
        if (!$organization) {
            return response()->json(['message' => 'Organization not found'], 404);
