...
class WebinarController extends Controller
{
    ...
    {
        $typeOptions = [];
        $types = ['webinar', 'course', 'text_lesson', 'graduation_project'];

        foreach ($types as $type) {
            $typeOptions[] = [
                'label_en' => $type,
                'label_ar' => trans('panel.' . $type, [], 'ar'),
            ];
        }
    ...
    public function index($url_name, Request $request)
    {
        ...
        if ($type) {
            // If type is provided in URL
            $query = Webinar::where('webinars.type', $type);
        } else {
            // If no type in URL → get all
            $query = Webinar::query();
        }
        ...
        if (!empty($teacher)) {
            $query->whereHas('teacher', function ($q) use ($teacher) {
                $q->where('full_name', 'like', '%' . $teacher . '%');
            });
        }
        ...
        foreach ($webinars as $webinar) {
            ...
            $sales = Sale::query()
                ->whereNull('refund_at')
                ->where(function ($q) use ($webinar, $giftsIds) {
                    $q->where('webinar_id', $webinar->id);
                    if (!empty($giftsIds)) {
                        $q->orWhereIn('gift_id', $giftsIds);
                    }
                })
                ->get();

            $webinar->sales = $sales->count();
            $webinar->sales_amount = $sales->sum('total_amount');
             if (!empty($webinar->translations)) {
    ...
    public function store($url_name, Request $request)
    {
        try {
            ...
            $this->validate($request, [
                ...
                'course_name_certificate' => 'required|string|max:255',
                ...
                'unattached' => 'required|numeric',
                'hasGroup' => 'nullable',
                'start_date' => 'nullable|date|required_if:required,webinar',
                'timezone' => 'nullable'
            ]);
            ...
            $webinar = Webinar::create([
                ...
                'thumbnail' => $data['thumbnail'] ?? "test",
                'image_cover' => $data['image_cover'] ?? "test",
    ...
    public function update($url_name, Request $request, $id)
    {
        try {
            ...
            $rules = [
                'type' => 'sometimes|in:webinar,course,text_lesson,graduation_project',
    ...
    public function approve($url_name, $id)
    {
        ...
        if ($webinar->status !== Webinar::$pending) {
            return response()->json(['message' => 'Webinar should be pending'], 400);
        }
    ...
    public function unpublish($url_name, $id)
    {
        ...
        if ($webinar->status !== Webinar::$active) {
            return response()->json(['message' => 'Webinar should be active'], 400);
        }
    ...
    public function studentsLists($url_name, Request $request, $id)
    {
        ...
        if (!empty($webinar)) {
            ...
            foreach ($students as $key => $student) {
                ...
                $usersGroup = $student->getUserGroup();
            }
    ...
    public function statistics()
    {
        ...
        $webinars = Webinar::where('unattached', 1)->paginate($perPage);
