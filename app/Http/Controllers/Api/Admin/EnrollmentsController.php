...
class EnrollmentsController extends Controller
{
    ...
    public function history(Request $request)
    {
        ...
        $statusLabelFilter = $request->input('status_label');
        $page = $request->get('page', 1);
        $perPage = 10;

        if ($fromDateFilter) {
            $fromTimestamp = strtotime($fromDateFilter . '00:00:00');
            $salesQuery->where('created_at', '>=', $fromTimestamp);
        }

        if ($toDateFilter) {
            $toTimestamp = strtotime($toDateFilter . ' 23:59:59');
            $salesQuery->where('created_at', '<=', $toTimestamp);
        }

        $sales = $salesQuery->orderBy('created_at', 'desc')
            ->with(['buyer', 'webinar'])
            ->whereHas('webinar.translation', function ($query) use ($filters) {
                if ($filters) {
                    $query->filterBySearch(['title' => $filters]);
                }
            })
            ->whereHas('seller', function ($query) use ($teacherNameFilter) {
                if ($teacherNameFilter) {
                    $query->filterBySearch(['full_name' => $teacherNameFilter]);
                }
            })
            ->whereHas('buyer', function ($query) use ($studentNameFilter) {
        ...
        $filtered = $sales->map(function ($sale) {
            $sale = $this->makeTitle($sale);
            $sale->invoice_url = 'https://api.lxera.net/admin/financial/sales/' . $sale->id . '/invoice';

            if (!empty($sale->refund_at)) {
                $sale->status_label = 'refund';
            } elseif (!$sale->access_to_purchased_item) {
                $sale->status_label = 'blocked';
            } else {
                $sale->status_label = 'success';
            }
        ...
        if (!empty($statusLabelFilter)) {
            $filtered = $filtered->filter(function ($sale) use ($statusLabelFilter) {
                return $sale->status_label === $statusLabelFilter;
            });
        }
        ...
        $cleanSales = $paginated->map(function ($sale) {
            $array = $sale->toArray();
        ...
        if (!empty($teacher_ids)) {
            $extraData['teachers'] = User::select('id', 'full_name')
                ->whereIn('id', $teacher_ids)->get();
        }

        if (!empty($student_ids)) {
            $extraData['students'] = User::select('id', 'full_name')
                ->whereIn('id', $student_ids)->get();
        }

        if (!empty($webinar_ids)) {
            $extraData['webinars'] = Webinar::select('id')
