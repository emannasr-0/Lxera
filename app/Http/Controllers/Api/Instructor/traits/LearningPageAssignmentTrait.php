...
namespace App\Http\Controllers\Api\Instructor\traits;
...
trait LearningPageAssignmentTrait
{
    ...
    private function getAssignmentData($course, $requestData)
    {
        ...
        if (!empty($assignment)) {
            ...
            if (!empty($webinar) and $webinar->checkUserHasBought($user)) {
                ...
                $studentId = (!empty($requestData['student']) and ($webinar->isOwner($user->id) or $webinar->isPartnerTeacher($user->id))) ? $requestData['student'] : null;
    ...
    private function getAssignmentHistory($course, $assignment, $user, $studentId = null)
    {
        ...
        //     $existingAssignmentHistory = WebinarAssignmentHistory::
        //     where('student_id', $user->id)
        //     ->where('assignment_id', $assignment->id)
        //     ->first();

        // if (!$existingAssignmentHistory) {
        //     // Only create the assignment history if it doesn't already exist
        //     $assignmentHistory = WebinarAssignmentHistory::create([
        //         'instructor_id' => $assignment->creator_id,
        //         'student_id' => $user->id,
        //         'assignment_id' => $assignment->id,
        ...
        //     ]);
        // } else {
        //     // If it already exists, use the existing record
        //     $assignmentHistory = $existingAssignmentHistory;
        // }
        }
