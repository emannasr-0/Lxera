...
class UsersController extends Controller
{
    ...
    public function show($url_name, $id)
    {
        ...
        $user = User::where('id', $id)->with(['Student'])->get();
    ...
    public function directRegister(Request $request, $is_export_excel = false)
    {
        ...
        $query = BundleStudent::whereHas('student')
            ->whereNull('class_id')
    ...
    public function update(Request $request, $url_name, $id)
    {
        ...
        try {
            ...
            $rules = [
                'full_name' => 'sometimes|string|min:3|max:128',
    ...
    private function getStatusOptions()
    {
        ...
        foreach ($statuses as $status) {
            $statusOptions[] = [
                'label_en' => trans('panel.' . $status, [], 'en'),
                'label_ar' => trans('panel.' . $status, [], 'ar'),
            ];
        }
