...
class CartController extends Controller
{
    ...
    public function validateCoupon(Request $request)
    {
        ...
        if (!empty($discountCoupon)) {
            ...
            if ($checkDiscount != 'ok') {
                return sendError([], $checkDiscount, 422);
            }
            ...
            if (!empty($order)) {
                ...
                if (!empty($calculate)) {
                    $data = [
                        ...
                        'total_discount' =>  (int)round($calculate["total_discount"]),
                        'total_tax' =>  (int)round($calculate["tax_price"]),
                        'amount' =>  (int)round($calculate["total_amount"]),
                        'total_amount' =>  (int)round($calculate["total_amount"] - $calculate["total_discount"]),
                        'status' => 200
                    ];
                    return sendResponse($data, trans('cart.coupon_valid'));
                }
            }
        }
        return sendError([], trans('cart.coupon_invalid'), 422);
    }
