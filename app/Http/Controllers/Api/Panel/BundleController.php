...
public function purchases(Request $request)
{
    ...
    // Load sales with required relations
        ...
        ->with([
            ...
            'bundle' => function ($query) {
                $query->with([
                    ...
                    'category',
                    'teacher:id,full_name',
                    'bundleWebinars.webinar.assignments.assignmentHistory',
                    'bundleWebinars.webinar.translations',
                ]);
            }
        ])
    ...
    foreach ($sales as $sale) {
        if (!empty($sale->gift_id)) {
            ...
            $sale->webinar = $gift->webinar_id ? $gift->webinar : null;
            $sale->bundle = $gift->bundle_id ? $gift->bundle : null;

            $sale->gift_recipient = $gift->receipt->full_name ?? $gift->name;
            ...
            if (!empty($sale->webinar)) {
                $giftDurations += $sale->webinar->duration ?? 0;
            ...
            if (!empty($sale->bundle)) {
                foreach ($sale->bundle->bundleWebinars as $bw) {
                    $giftDurations += $bw->webinar->duration ?? 0;
                }
            }
        }
    }

    // Assignment summary per bundle
    $bundles = $sales->map(function ($sale) {
        $bundle = $sale->bundle;

        if ($bundle && $bundle->relationLoaded('bundleWebinars')) {
            $assignmentStats = $bundle->bundleWebinars->map(function ($bw) {
                $webinar = $bw->webinar;

                if (!$webinar || !$webinar->relationLoaded('assignments')) {
                    return null;
                }

                $assignments = $webinar->assignments ?? collect();
                $total = $assignments->count();
                $submitted = $assignments->filter(fn($a) => $a->assignmentHistory)->count();
                $remaining = $total - $submitted;

                // Prefer translated title if exists
                $title = $webinar->title;
                if (!$title && $webinar->translations && $webinar->translations->first()) {
                    $title = $webinar->translations->first()->title;
                }
            ...
            $bundle->assignment_summary = $assignmentStats->values();
        }

        $resource = BundleResource::make($bundle);
        $resource->additional(['assignment_summary' => $bundle->assignment_summary]);
    ...
    $webinarsHours = deepClone($query)
        ->join('webinars', 'webinars.id', 'sales.webinar_id')
        ->sum('webinars.duration');

    $bundlesHours = deepClone($query)
        ->join('bundle_webinars', 'bundle_webinars.bundle_id', 'sales.bundle_id')
        ->join('webinars', 'webinars.id', 'bundle_webinars.webinar_id')
        ->sum('webinars.duration');

    $upComing = deepClone($query)
        ->join('webinars', 'webinars.id', 'sales.webinar_id')
    ...
    ], "All purchased bundles are retrieved");
}
