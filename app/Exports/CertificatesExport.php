...
class CertificatesExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function map($certificate): array
    {
        return [
            $certificate->id,
            optional($certificate->quiz)->title,
            optional(optional($certificate->quiz)->webinar)->title,
            optional($certificate->student)->full_name ?? trans('update.deleted_user'),
            optional(optional($certificate->quiz)->teacher)->full_name,
            optional($certificate->quizzesResult)->user_grade ?? '-',
