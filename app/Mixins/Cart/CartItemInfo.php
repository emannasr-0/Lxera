...
class CartItemInfo
{
    public function getItemInfo($cart)
    {
        ...
        }elseif (!empty($cart->certificate_template_id)) {
            $certificate = $cart->certificate_template;

            return $this->getCertificateInfo($cart, $certificate);
        }
        // elseif (!empty($cart->form_fee)) {
        //     return $this->getForm_feeInfo($cart);
        // }
    ...
    // private function getForm_feeInfo($cart)
    // {
    //     $info = [];

    //     $info['imgPath'] = null;
    //     $info['itemUrl'] = null;
    //     $info['title'] = "رسوم التسجيل ";
    ...
    //     return $info;
    // }
    private function getCertificateInfo($cart, $certificate)
    {
        $info = [];

        $info['imgPath'] = $certificate->getImage($certificate->id);
        $info['itemUrl'] = null;
        $info['title'] = " شهادة  ".$certificate->title;
        $info['profileUrl'] = null;
        $info['teacherName'] = null;
        $info['rate'] = null;
        $info['price'] = $certificate->price;
        $info['discountPrice'] = null;
