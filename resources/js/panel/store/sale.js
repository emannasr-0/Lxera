(function () {
    ...
    function makeEnterTrackingCodeModalHtml(order, saleId) {
        return `
            <div>
                <h3 class="section-title after-line font-20 text-light mb-25">${enterTrackingCodeModalTitleLang}</h3>
    ...
    $('body').on('click', '#saveTrackingCode', function (e) {
        ...
        $.post(action, data, function (result) {
            if (result && result.code === 200) {
                Swal.fire({
                    icon: 'success',
                    html: '<h3 class="font-20 text-center text-light py-25">' + trackingCodeSaveSuccessLang + '</h3>',
                    showConfirmButton: false,
                    width: '25rem',
                });
