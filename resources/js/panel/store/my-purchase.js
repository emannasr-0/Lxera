(function () {
    ...
    $('body').on('click', '.js-view-tracking-code', function () {
        ...
        $.get(path, function (result) {
            if (result && result.order) {
                ...
                const html = `
                    <div>
                        <h3 class="section-title after-line font-20 text-light mb-25">${viewTrackingCodeModalTitleLang}</h3>
    ...
    $('body').on('click', '.js-got-the-parcel', function () {
        ...
        const html = `
                    <div>
                        <h3 class="section-title after-line font-20 text-light mb-25">${gotTheParcelLang}</h3>
    ...
    $('body').on('click', '#saveGotTheParcel', function (e) {
        ...
        $.get(path, function (result) {
            if (result && result.code === 200) {
                Swal.fire({
                    icon: 'success',
                    html: '<h3 class="font-20 text-center text-light py-25">' + gotTheParcelSaveSuccessLang + '</h3>',
                    showConfirmButton: false,
                    width: '25rem',
                });
                ...
                Swal.fire({
                    icon: 'error',
                    html: '<h3 class="font-20 text-center text-light py-25">' + gotTheParcelSaveErrorLang + '</h3>',
                    showConfirmButton: false,
                    width: '25rem',
                });
            }
