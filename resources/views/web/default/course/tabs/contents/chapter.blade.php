<div class=" js-font-resize row">
    <div class=" js-font-resize col-12">
        <div class=" js-font-resize accordion-content-wrapper" id="chaptersAccordion" role="tablist" aria-multiselectable="true">
            @foreach($course->chapters as $chapter)
                ...
                @if((!empty($chapter->chapterItems) and count($chapter->chapterItems)) or (!empty($chapter->quizzes) and count($chapter->quizzes)))
                    <div class=" js-font-resize accordion-row rounded-sm border mt-20 p-15">
                        <div class=" js-font-resize d-flex align-items-center justify-content-between" role="tab" id="chapter_{{ $chapter->id }}">
                            <div class=" js-font-resize js-chapter-collapse-toggle d-flex align-items-center" href="#collapseChapter{{ $chapter->id }}" aria-controls="collapseChapter{{ $chapter->id }}" data-parent="#chaptersAccordion" role="button" data-toggle="collapse" aria-expanded="true">
                                <span class=" js-font-resize chapter-icon mr-15">
                                    <i data-feather="grid" class=" js-font-resize "></i>
                                </span>

                                <span class=" js-font-resize font-weight-bold text-secondary font-14">{{ $chapter->title }}</span>
                            </div>

                            <div class=" js-font-resize d-flex align-items-center">
                                <span class=" js-font-resize mr-15 font-14 text-gray">
                                ...
                                <i class=" js-font-resize collapse-chevron-icon" data-feather="chevron-down" height="20" href="#collapseChapter{{ !empty($chapter) ? $chapter->id :'record' }}" aria-controls="collapseChapter{{ !empty($chapter) ? $chapter->id :'record' }}" data-parent="#chaptersAccordion" role="button" data-toggle="collapse" aria-expanded="true"></i>
                            </div>
                        </div>
