<div class=" js-font-resize mt-35">
    <div class=" js-font-resize course-reviews-box row align-items-center">
        <div class=" js-font-resize col-3 text-center">
            <div class=" js-font-resize reviews-rate font-36 font-weight-bold text-primary">{{ $course->getRate() }}</div>
            ...
            <div class=" js-font-resize text-center">
                ...
                <div class=" js-font-resize mt-15">{{ $course->reviews->pluck('creator_id')->count() }}  {{ trans('product.reviews') }}</div>
            </div>
        </div>
        ...
        <div class=" js-font-resize col-9">
            <div class=" js-font-resize d-flex align-items-center">
                <div class=" js-font-resize progress course-progress rounded-sm">
                    <span class=" js-font-resize progress-bar rounded-sm" style="width: {{ $course->reviews->avg('content_quality') / 5 * 100 }}%"></span>
                </div>
                <span class=" js-font-resize ml-15 font-14 text-gray text-left">{{ trans('product.content_quality') }} ({{ $course->reviews->count() > 0 ? round($course->reviews->avg('content_quality'), 1) : 0 }})</span>
            </div>

            <div class=" js-font-resize mt-25 d-flex align-items-center">
                <div class=" js-font-resize progress course-progress rounded-sm">
                    <span class=" js-font-resize progress-bar rounded-sm" style="width: {{ $course->reviews->avg('instructor_skills') / 5 * 100 }}%"></span>
                </div>
                <span class=" js-font-resize ml-15 font-14 text-gray text-left">{{ trans('product.instructor_skills') }} ({{ $course->reviews->count() > 0 ? round($course->reviews->avg('instructor_skills'), 1) : 0 }})</span>
            </div>

            <div class=" js-font-resize mt-25 d-flex align-items-center">
                <div class=" js-font-resize progress course-progress rounded-sm">
                    <span class=" js-font-resize progress-bar rounded-sm" style="width: {{ $course->reviews->avg('purchase_worth') / 5 * 100 }}%"></span>
                </div>
                <span class=" js-font-resize ml-15 font-14 text-gray text-left">{{ trans('product.purchase_worth') }} ({{ $course->reviews->count() > 0 ? round($course->reviews->avg('purchase_worth'), 1) : 0 }})</span>
            </div>

            <div class=" js-font-resize mt-25 d-flex align-items-center">
                <div class=" js-font-resize progress course-progress rounded-sm">
                    <span class=" js-font-resize progress-bar rounded-sm" style="width: {{ $course->reviews->avg('support_quality') / 5 * 100 }}%"></span>
                </div>
                <span class=" js-font-resize ml-15 font-14 text-gray text-left">{{ trans('product.support_quality') }} ({{ $course->reviews->count() > 0 ? round($course->reviews->avg('support_quality'), 1) : 0 }})</span>
            </div>
...
<section class=" js-font-resize mt-40">
    <h2 class=" js-font-resize section-title after-line">{{ trans('product.reviews') }} ({{ $course->reviews->pluck('creator_id')->count() }})</h2>

    <form action="/reviews/store" class=" js-font-resize mt-20" method="post">
        {{ csrf_field() }}
    ...
    <div class=" js-font-resize mt-45">
        @if($course->reviews->count() > 0)
            @foreach($course->reviews as $review)
                ...
                <div class=" js-font-resize comments-card shadow-lg rounded-sm border px-20 py-15 mt-30" data-address="/reviews/store-reply-comment" data-csrf="{{ csrf_token() }}" data-id="{{ $review->id }}">
                        ...
                        <div class=" js-font-resize d-flex align-items-center">
                            ...
                            <div class=" js-font-resize btn-group dropdown table-actions">
                                ...
                                <div class=" js-font-resize dropdown-menu">
                                    <a href="/reviews/store-reply-comment" class=" js-font-resize webinar-actions d-block text-hover-primary reply-comment">{{ trans('panel.reply') }}</a>

                                    @if(!empty($user) and $user->id == $review->creator_id)
                                        <a href="/reviews/{{ $review->id }}/delete" class=" js-font-resize webinar-actions d-block mt-10 text-hover-primary">{{ trans('public.delete') }}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
