<div class=" js-font-resize product-show-seller-tab mt-20">

    <div class=" js-font-resize product-show__profile-info-box profile-info-box d-flex align-items-start justify-content-between">
        <div class=" js-font-resize user-details d-flex align-items-center">
            <div class=" js-font-resize user-profile-avatar">
                <img src="{{ $seller->getAvatar(190) }}" class=" js-font-resize img-cover" alt="{{ $seller->full_name }}"/>
            ...
            <div class=" js-font-resize ml-20 ml-lg-40">
                <h1 class=" js-font-resize font-24 font-weight-bold text-light">{{ $seller->full_name }}</h1>
                <span class=" js-font-resize text-gray">{{ $seller->headline }}</span>
                ...
                <div class=" js-font-resize w-100 mt-10 d-flex align-items-center justify-content-center justify-content-lg-start">
                    <div class=" js-font-resize d-flex flex-column followers-status">
                        <span class=" js-font-resize font-20 font-weight-bold text-dark-blue">{{ $sellerFollowers->count() }}</span>
                    ...
                    <div class=" js-font-resize d-flex flex-column ml-25 pl-5 following-status">
                        <span class=" js-font-resize font-20 font-weight-bold text-dark-blue">{{ $sellerFollowing->count() }}</span>
                ...
                @if(!empty($sellerBadges))
                    <div class=" js-font-resize user-reward-badges d-flex flex-wrap align-items-center mt-15">
                        @foreach($sellerBadges as $sellerBadge)
                            <div class=" js-font-resize mr-15" data-toggle="tooltip" data-placement="bottom" data-html="true" title="{!! (!empty($sellerBadge->badge_id) ? nl2br($sellerBadge->badge->description) : nl2br($sellerBadge->description)) !!}">
        ...
        <div class=" js-font-resize user-actions d-flex flex-column">
            <button type="button" id="followToggle" data-user-id="{{ $seller->id }}" class=" js-font-resize btn btn-{{ (!empty($authUserIsFollower) and $authUserIsFollower) ? 'danger' : 'primary' }} btn-sm">
    ...
    @if($seller->offline)
        <div class=" js-font-resize user-offline-alert d-flex mt-40">
            <div class=" js-font-resize p-15">
                <h3 class=" js-font-resize font-16 text-dark-blue">{{ trans('public.instructor_is_not_available') }}</h3>
                <p class=" js-font-resize font-14 font-weight-500 text-gray mt-15">{{ $seller->offline_message }}</p>
            </div>

            <div class=" js-font-resize offline-icon offline-icon-right ml-auto d-flex align-items-stretch">
    ...
    @if(!empty($seller->about))
        <div class=" js-font-resize mt-40">
            <h3 class=" js-font-resize font-16 text-dark-blue font-weight-bold">{{ trans('site.about') }}</h3>

            <div class=" js-font-resize mt-15 text-gray">
                {!! nl2br($seller->about) !!}
            </div>
        </div>
    @endif
