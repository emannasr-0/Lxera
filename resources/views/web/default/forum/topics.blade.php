...
@section('content')
    <div class=" js-font-resize container">
        <section class=" js-font-resize topics-title-section mt-30 mt-md-50 px-20 px-md-30 py-25 py-md-35 rounded-lg">
            <h1 class=" js-font-resize font-30 font-weight-bold text-white">{{ $forum->title }}</h1>
            <p class=" js-font-resize font-14 text-white">{{ $forum->description }}</p>
            ...
            <div class=" js-font-resize mt-10">
                <nav aria-label="breadcrumb">
                    <ol class=" js-font-resize breadcrumb p-0 m-0">
                        ...
                        <li class=" js-font-resize breadcrumb-item font-12 text-white font-weight-bold" aria-current="page">{{ $forum->title }}</li>
                    </ol>
                </nav>
            </div>
        </section>
        ...
        <div class=" js-font-resize topics-filters-section bg-white rounded-lg px-20 py-25 mt-40">
            <div class=" js-font-resize row">
                <div class=" js-font-resize col-12 col-md-5">
                    <h3 class=" js-font-resize font-16 font-weight-bold text-secondary">{{ trans('update.topics_in_this_forum',['count' => $topics->count()]) }}</h3>

                    <div class=" js-font-resize d-flex align-items-center mt-5">
                        <a href="{{ request()->url() }}?sort=newest" class=" js-font-resize font-14 font-weight-500 mr-20 {{ (empty(request()->get('sort')) or request()->get('sort') == 'newest') ? 'text-primary' : 'text-gray' }}">{{ trans('public.newest') }}</a>
                        <a href="{{ request()->url() }}?sort=popular_topics" class=" js-font-resize font-14 font-weight-500 mr-20 {{ (request()->get('sort') == 'popular_topics') ? 'text-primary' : 'text-gray' }}">{{ trans('update.popular_topics') }}</a>
                        <a href="{{ request()->url() }}?sort=not_answered" class=" js-font-resize font-14 font-weight-500 {{ (request()->get('sort') == 'not_answered') ? 'text-primary' : 'text-gray' }}">{{ trans('update.not_answered') }}</a>
                    </div>
                </div>
                ...
                <div class=" js-font-resize col-12 col-md-7  mt-15 mt-lg-0">
                    <div class=" js-font-resize row">
                        <div class=" js-font-resize col-12 col-lg-7">
                            <form action="" method="get">
                                <div class=" js-font-resize d-flex align-items-center">
                                    <input type="text" name="search" value="{{ request()->get('search') }}" class=" js-font-resize form-control flex-grow-1 input-search-topic" placeholder="{{ trans('update.search_in_this_forum') }}">
                                    <button type="submit" class=" js-font-resize btn btn-sm btn-primary btn-search-topic ml-10">
                                        <i data-feather="search" class=" js-font-resize text-white" width="20" height="20"></i>
                                    </button>
                                </div>
                            </form>
                        </div>
                        <div class=" js-font-resize col-12 col-lg-5 mt-15 mt-lg-0 text-right">
                            @if($forum->close)
                                <button type="button" class=" js-font-resize btn btn-primary btn-create-topic disabled btn-block" disabled>
                                    <i data-feather="file" class=" js-font-resize text-white" width="16" height="16"></i>
                                    <span class=" js-font-resize ml-1">{{ trans('update.forum_closed') }}</span>
                                </button>
                            @else
                                <a href="/forums/create-topic{{ $forum->checkUserCanCreateTopic() ? '?forum_id='.$forum->id : '' }}" class=" js-font-resize btn btn-primary btn-create-topic btn-block">
                                    <i data-feather="file" class=" js-font-resize text-white" width="16" height="16"></i>
                ...
                <div class=" js-font-resize col-12 col-md-9">
                                            ...
                                            <div class=" js-font-resize d-flex align-items-center">
                                                ...
                                                <div class=" js-font-resize ml-10 mw-100">
                                                    <a href="{{ $topic->getPostsUrl() }}" class=" js-font-resize ">
                                                        <h4 class=" js-font-resize font-14 font-weight-bold text-secondary text-ellipsis">{{ $topic->title }}</h4>
                                                    </a>
                                                    <span class=" js-font-resize d-block font-12 mt-5 text-gray">{{ trans('public.by') }} {{ $topic->creator->full_name }} {{ trans('public.in') }} {{ dateTimeFormat($topic->created_at,'j M Y | H:i') }}</span>
                                                </div>
                                            </div>
                                        </div>

                                        <div class=" js-font-resize col-12 col-md-6 d-none d-md-block">
                                            <div class=" js-font-resize row">
                                                    ...
                                                    @if(!empty($topic->lastPost))
                                                        <div class=" js-font-resize d-flex align-items-center">
                                                            ...
                                                            <div class=" js-font-resize ml-10">
                                                                <h4 class=" js-font-resize font-12 font-weight-500 text-gray">{{ $topic->lastPost->user->full_name }}</h4>
                    ...
                    @else
                        <div class=" js-font-resize topics-not-result d-flex align-items-center justify-content-center flex-column">
                            ...
                            <div class=" js-font-resize d-flex align-items-center flex-column mt-10 text-center">
                                <h3 class=" js-font-resize font-20 font-weight-bold text-light text-center">{{ trans('update.result_not_found') }}</h3>
                                <p class=" js-font-resize font-14 font-weight-500 text-gray mt-5 text-center">{{ trans('update.you_not_access_to_this_forum') }}</p>
                            </div>
                        </div>
                    @endif
                </div>
                    ...
                    @if(!empty($popularTopics) and count($popularTopics))
                        <div class=" js-font-resize rounded-lg p-15 border bg-white mt-20">
                            ...
                            @foreach($popularTopics as $popularTopic)
                                <div class=" js-font-resize d-flex align-items-center mt-15">
                                    <div class=" js-font-resize topics-right-side-user-avatar rounded-circle">
                                        <img src="{{ !empty($popularTopic->creator) ? $popularTopic->creator->getAvatar(48) : '' }}" class=" js-font-resize img-cover rounded-circle" alt="{{ !empty($popularTopic->creator) ? $popularTopic->creator->full_name : '' }}">
                                    </div>
                                    <div class=" js-font-resize ml-10">
                                        <a href="{{ $popularTopic->getPostsUrl() }}" class=" js-font-resize d-block pb-5">
