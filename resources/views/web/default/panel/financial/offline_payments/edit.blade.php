...
<!-- Modal -->
<div class=" js-font-resize modal fade" id={{ 'confirmModal' . $id }} tabindex="-1" aria-labelledby="confirmModalLabel"
    aria-hidden="true" data-confirm-href="{{ $url }}">
    <div class=" js-font-resize modal-dialog">
        <div class=" js-font-resize modal-content">
            <div class=" js-font-resize modal-header align-items-baseline">
                <h5 class=" js-font-resize modal-title" id="confirmModalLabel">{{ 'تعديل الطلب واعادة ارساله ' }}</h5>
                <button type="button" class=" js-font-resize close m-0" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form class=" js-font-resize modal-body" method="POST" action="{{ $url }}" id="deleteForm" enctype="multipart/form-data">
                <div class=" js-font-resize row">
                    @csrf


                    <div class=" js-font-resize col-12 col-md-6 mb-25 mb-lg-0 js-offline-payment-input ">
                        <div class=" js-font-resize form-group text-left">
                    ...
                    <div class=" js-font-resize col-12 col-md-6 mb-25 mb-lg-0 js-offline-payment-input ">
                        <div class=" js-font-resize form-group text-left">
                            <label for="IBAN" class=" js-font-resize input-label"> اي بان (IBAN)</label>
                            <input type="text" name="IBAN" id="IBAN" value="{{ old('IBAN', $payment->iban) }}"
                                class=" js-font-resize form-control @error('IBAN') is-invalid @enderror" />
                            @error('IBAN')
                                <div class=" js-font-resize invalid-feedback"> {{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class=" js-font-resize col-12 mb-25 mb-lg-0 js-offline-payment-input ">
                        <div class=" js-font-resize form-group text-left">
                            ...
                            {{-- <label for="attachmentFile" id="attachmentFileLabel{{$id}}"
                                class=" js-font-resize custom-upload-input-group flex-row-reverse ">
                            ...
                            <input type="file" name="attachment" id="attachmentFile{{$id}}" accept=".jpeg,.jpg,.png"
                                class=" js-font-resize form-control h-auto @error('attachment') is-invalid @enderror"
                                value="" />
                            @error('attachment')
                ...
                <div class=" js-font-resize modal-footer">
                    ...
                    <button type="submit"
                        class=" js-font-resize btn btn-danger id="confirmAction">{{ trans('admin/main.send') }}</button>
                </div>
            </form>
        </div>
    </div>
