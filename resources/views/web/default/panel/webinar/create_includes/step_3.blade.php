    ...
    <div class=" js-font-resize col-12 col-md-6">
        ...
        <div class=" js-font-resize form-group mt-30 d-flex align-items-center justify-content-between mb-5">
            ...
            <div class=" js-font-resize custom-control custom-switch">
                <input type="checkbox" name="subscribe" class=" js-font-resize custom-control-input" id="subscribeSwitch" {{ !empty($webinar) && $webinar->subscribe ? 'checked' : (old('subscribe') ? 'checked' : '')  }}>
        ...
        <div class=" js-font-resize form-group mt-15">
            ...
            <input type="number" name="access_days" value="{{ !empty($webinar) ? $webinar->access_days : old('access_days') }}" class=" js-font-resize form-control @error('access_days')  is-invalid @enderror"/>
            @error('access_days')
        ...
        <div class=" js-font-resize form-group mt-15">
            ...
            <input type="number" name="price" value="{{ (!empty($webinar) and !empty($webinar->price)) ? convertPriceToUserCurrency($webinar->price) : old('price') }}" class=" js-font-resize form-control @error('price')  is-invalid @enderror" placeholder="{{ trans('public.0_for_free') }}"/>
            @error('price')
            <div class=" js-font-resize invalid-feedback">
                {{ $message }}
            </div>
        ...
        @if($authUser->isOrganization() and $authUser->id == $webinar->creator_id)
            <div class=" js-font-resize form-group mt-15">
                <label class=" js-font-resize input-label">{{ trans('update.organization_price') }} ({{ $currency }})</label>
                <input type="number" name="organization_price" value="{{ (!empty($webinar) and $webinar->organization_price) ? convertPriceToUserCurrency($webinar->organization_price) : old('organization_price') }}" class=" js-font-resize form-control @error('organization_price')  is-invalid @enderror" placeholder=""/>
                @error('organization_price')
                <div class=" js-font-resize invalid-feedback">
                    {{ $message }}
                </div>
                @enderror
                <p class=" js-font-resize font-12 text-gray mt-5">- {{ trans('update.organization_price_hint') }}</p>
            </div>
        @endif
    </div>
</div>
...
<section class=" js-font-resize mt-30">
    ...
    <button id="webinarAddTicket" data-webinar-id="{{ $webinar->id }}" type="button" class=" js-font-resize btn btn-primary btn-sm mt-15">{{ trans('public.add_plan') }}</button>
