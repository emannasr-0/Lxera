...
<section class=" js-font-resize row mx-0 col-12">
    {{-- <div class=" js-font-resize col-12 text-center mb-20">
        @if (!($hasBought or !empty($bundleData['bundle']->bundle->getInstallmentOrder())) && !empty($bundleData['bundle']->studentRequirement))
            <p class=" js-font-resize alert alert-success text-center">
    ...
    @if ($hasBought)
        ...
        <div class=" js-font-resize col-12 mb-md-0 mb-20">
                ...
                <div class=" js-font-resize row">
                    <div class=" js-font-resize col-12">
                        ...
                        <div class=" js-font-resize ">
                            <p class=" js-font-resize text-dark font-14 text-ellipsis">
                                {{ $subTitle }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class=" js-font-resize col-12 p-0">
                        <div class=" js-font-resize  d-flex flex-column w-100 pt-0">


                            <div class=" js-font-resize order-2 col-12">

                                <div class=" js-font-resize mt-20 d-flex flex-column">

                                    <button type="button" class=" js-font-resize btn btn-acadima-primary"
    ...
    @else
        {{-- direct buy --}}
        <div
            class=" js-font-resize col-12 mb-md-0 mb-20 {{ !empty($bundleData['installments']) && count($bundleData['installments']) ? 'col-md-6' : '' }}">
            <div class=" js-font-resize installment-card p-15 w-100 h-100">
                <div class=" js-font-resize row">
                    <div class=" js-font-resize col-12">
                    ...
                    <div class=" js-font-resize col-12 p-0">
                        <div class=" js-font-resize installment-card__payments d-flex flex-column w-100 pt-0">

                            <div class=" js-font-resize d-flex align-items-center justify-content-center flex-column order-1">
                                @if ($bundleData['bundle']->bundle->price > 0)
                                    <div id="priceBox"
                                        class=" js-font-resize order-1 col-12 text-center d-flex align-items-center justify-content-center mt-20 {{ !empty($activeSpecialOffer) ? ' flex-column ' : '' }}">
                                        <div class=" js-font-resize text-center">
                                            ...
                                            @if (!($hasBought or !empty($bundleData['bundle']->bundle->getInstallmentOrder())))
                                                ...
                                                <span id="realPrice"
                                                    ...
                                                    class=" js-font-resize d-block @if (!empty($activeSpecialOffer)) font-16 text-gray text-decoration-line-through @else font-36 text-primary @endif">
                                                    {{ $realPrice['price'] }}
                                                </span>
                                            ...
                                            @if (!empty($realPrice['tax']) and empty($activeSpecialOffer))
                                                <span class=" js-font-resize d-block font-14 text-gray">+
                                                    {{ $realPrice['tax'] }}
                                                    tax</span>
                                            @endif
                                        </div>
                                ...
                                @else
                                    <div class=" js-font-resize d-flex align-items-center justify-content-center mt-20 order-1 col-12 ">
                            ...
                            <div class=" js-font-resize  mb-15 order-3">
                                <div class=" js-font-resize d-flex align-items-center font-12 text-gray">
                                    @if (!($hasBought or !empty($bundleData['bundle']->bundle->getInstallmentOrder())))
                                        <section class=" js-font-resize bundle-details mt-3 order-3 col-12">
                                            <p class=" js-font-resize bundle-details text-dark mt-10">
                                                <!-- {{ trans('panel.online_program_100') }} -->
                                                  Single payment to enroll in the program
                                            </p>

                                            {{-- <p class=" js-font-resize bundle-details text-pink mt-10">
                                            مكونة من
                            ...
                            <form action="{{ route('purchase_bundle') }}" method="POST" class=" js-font-resize order-2 col-12">
                                {{ csrf_field() }}
                                ...
                                <div class=" js-font-resize mt-20 d-flex flex-column">
                                    ...
                                    @elseif($bundleData['bundle']->bundle->price > 0)
                                        <button type="{{ $canSale ? 'submit' : 'button' }}"
                                            @if (!$canSale) disabled @endif class=" js-font-resize btn btn-acadima-primary">
                                            @if (!$canSale)
                                        ...
                                        @if ($canSale and !empty(getFeaturesSettings('direct_bundles_payment_button_status')))
                                            <button type="button"
                                                class=" js-font-resize btn btn-outline-danger js-bundle-direct-payment">
                                                {{ trans('update.buy_now') }}
                                            </button>
                                        @endif
                                    @else
                                        <a href="{{ $canSale ? '/bundles/' . $bundleData['bundle']->bundle->slug . '/free' : '#' }}"
                                            class=" js-font-resize btn btn-primary @if (!$canSale) disabled @endif">{{ trans('update.enroll_on_bundle') }}</a>
                                    @endif
                                </div>
        ...
        @if (empty($bundleData['bundle']->class_id) && empty($bundleData['bundle']->bridging))
            <div class=" js-font-resize text-center mx-auto mt-15">
                <p class=" js-font-resize text-pink font-weight-bold">{{trans('panel.or')}}</p>
                <p class=" js-font-resize text-dark font-14 text-ellipsis">Not ready to pay now, dont miss out reserve your place today and pay later </p>
                <a href="/panel/{{ $bundleData['bundle']->bundle->id }}/book_seat"
                    class=" js-font-resize btn btn-acadima-primary mx-auto mt-5">{{trans('panel.reserve_seat_here')}}  </a>
            </div>
        @endif
