...
@section('content')
    <div class=" js-font-resize ">
        ...
        <form method="post" action="/panel/store/products/{{ !empty($product) ? $product->id .'/update' : 'store' }}" id="productForm" class=" js-font-resize webinar-form">
        ...
        <div class=" js-font-resize create-webinar-footer d-flex flex-column flex-md-row align-items-center justify-content-between mt-20 pt-15 border-top">
            <div class=" js-font-resize d-flex align-items-center">

                @if(!empty($product))
                    <a href="/panel/store/products/{{ $product->id }}/step/{{ ($currentStep - 1) }}" class=" js-font-resize btn btn-sm btn-primary {{ $currentStep < 2 ? 'disabled' : '' }}">{{ trans('webinars.previous') }}</a>
                @else
                    <a href="" class=" js-font-resize btn btn-sm btn-primary disabled">{{ trans('webinars.previous') }}</a>
                @endif

                <button type="button" id="getNextStep" class=" js-font-resize btn btn-sm btn-primary ml-15" @if($currentStep >= 5) disabled @endif>{{ trans('webinars.next') }}</button>
            </div>
            ...
            <div class=" js-font-resize mt-20 mt-md-0">
                ...
                @if(!empty($product) and $product->creator_id == $authUser->id)
                    <a href="/panel/store/products/{{ $product->id }}/delete?redirect_to=/panel/store/products" class=" js-font-resize delete-action webinar-actions btn btn-sm btn-danger mt-20 mt-md-0">{{ trans('public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
