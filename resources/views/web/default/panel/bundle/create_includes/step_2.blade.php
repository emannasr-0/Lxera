...
<div class=" js-font-resize row">
    <div class=" js-font-resize col-12 col-md-6 mt-15">
        ...
        <div class=" js-font-resize form-group mt-15">
            ...
            <input type="text" name="tags" data-max-tag="5" value="{{ !empty($bundle) ? implode(',',$bundleTags) : '' }}" class=" js-font-resize form-control inputtags" placeholder="{{ trans('public.type_tag_name_and_press_enter') }} ({{ trans('forms.max') }} : 5)"/>
        </div>
...
<div class=" js-font-resize form-group mt-15 {{ (!empty($bundleCategoryFilters) and count($bundleCategoryFilters)) ? '' : 'd-none' }}" id="categoriesFiltersContainer">
                        ...
                        @foreach($filter->options as $option)
                            <div class=" js-font-resize form-group mt-10 d-flex align-items-center justify-content-between">
                                ...
                                <div class=" js-font-resize custom-control custom-checkbox">
                                    <input type="checkbox" name="filters[]" value="{{ $option->id }}" {{ ((!empty($bundleFilterOptions) && in_array($option->id, $bundleFilterOptions)) ? 'checked' : '') }} class=" js-font-resize custom-control-input" id="filterOptions{{ $option->id }}">
