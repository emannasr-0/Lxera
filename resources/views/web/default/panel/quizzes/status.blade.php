...
@section('content')
    <div class=" js-font-resize container">
        <section class=" js-font-resize mt-40">
            <h2 class=" js-font-resize font-weight-bold font-16 text-light">{{ trans('quiz.level_identification_quiz') }}</h2>
            <p class=" js-font-resize text-gray font-14 mt-5">{{ $quiz->title }} | {{ trans('public.by') }} <span class=" js-font-resize font-weight-bold">{{ $quiz->creator->full_name }}</span></p>
                ...
                <div class=" js-font-resize row">
                    ...
                    <div class=" js-font-resize col-6 col-md-3 mt-30 mt-md-0 d-flex align-items-center justify-content-center mt-5 mt-md-0">
                        <div class=" js-font-resize d-flex flex-column align-items-center text-center">
                            ...
                            <strong class=" js-font-resize font-30 font-weight-bold text-secondary mt-5">{{  $quizResult->user_grade }}/{{  $quizQuestions->sum('grade') }}</strong>
        ...
        <section class=" js-font-resize mt-30 rounded-lg shadow-sm py-25 px-20">
                ...
                @case(\App\Models\QuizzesResult::$passed)
                    <div class=" js-font-resize no-result default-no-result mt-50 d-flex align-items-center justify-content-center flex-column">
                        ...
                        <div class=" js-font-resize d-flex align-items-center flex-column mt-30 text-center">
                            <h2 class=" js-font-resize section-title">{{ trans('quiz.status_passed_title') }}</h2>
                            <p class=" js-font-resize mt-5 text-center">{!! trans('quiz.status_passed_hint',['grade' => $quizResult->user_grade.'/'.$quizQuestions->sum('grade')]) !!}</p>
                            ...
                            <div class=" js-font-resize  mt-25">
                                <a href="/panel/quizzes/my-results" class=" js-font-resize btn btn-sm btn-primary">{{ trans('public.show_results') }}</a>

                                @if($quiz->certificate)
                                    <a href="/panel/quizzes/results/{{ $quizResult->id }}/showCertificate" class=" js-font-resize btn btn-sm btn-primary">{{ trans('quiz.download_certificate') }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                @case(\App\Models\QuizzesResult::$failed)
                    <div class=" js-font-resize no-result status-failed mt-50 d-flex align-items-center justify-content-center flex-column">
                        <div class=" js-font-resize no-result-logo">
                            <img src="/assets/default/img/no-results/339.png" alt="">
                        </div>
                        <div class=" js-font-resize d-flex align-items-center flex-column mt-30 text-center">
                            <h2 class=" js-font-resize section-title">{{ trans('quiz.status_failed_title') }}</h2>
                            <p class=" js-font-resize mt-5 text-center">{!! trans('quiz.status_failed_hint',['min_grade' =>  $quiz->pass_mark .'/'. $quizQuestions->sum('grade'),'user_grade' => $quizResult->user_grade]) !!}</p>
                            @if($canTryAgain)
                                <p>{{ trans('public.you_can_try_again') }}</p>
                            @endif
                            <div class=" js-font-resize  mt-25">
                                @if($canTryAgain)
                                    <a href="/panel/quizzes/{{ $quiz->id }}/start" class=" js-font-resize btn btn-sm btn-primary">{{ trans('public.try_again') }}</a>
                                @endif
                                <a href="/panel/quizzes/my-results" class=" js-font-resize btn btn-sm btn-primary">{{ trans('public.show_results') }}</a>
                            </div>
                        </div>
                ...
                @case(\App\Models\QuizzesResult::$waiting)
                    <div class=" js-font-resize no-result status-waiting mt-50 d-flex align-items-center justify-content-center flex-column">
                        <div class=" js-font-resize no-result-logo">
                            <img src="/assets/default/img/no-results/242.png" alt="">
                        </div>
                        <div class=" js-font-resize d-flex align-items-center flex-column mt-30 text-center">
                            <h2 class=" js-font-resize section-title">{{ trans('quiz.status_waiting_title') }}</h2>
                            <p class=" js-font-resize mt-5 text-center">{!! nl2br(trans('quiz.status_waiting_hint')) !!}</p>
                            <div class=" js-font-resize  mt-25">
                                <a href="/panel/quizzes/my-results" class=" js-font-resize btn btn-sm btn-primary">{{ trans('public.show_results') }}</a>
                            </div>
                        </div>
