    ...
    <div class="row mt-20">
        <div class="col-12 col-lg-4">
            <div class="form-group">
                <label class="input-label">{{ trans('public.academic_code') }}</label>
                <input @if(!session()->has('impersonated') || empty($user->student)) disabled @endif type="text" name="user_code" value="{{  old('user_code', $user?->user_code ) }}" class="form-control @error('user_code')  is-invalid @enderror" placeholder=""/>
                @error('user_code')
