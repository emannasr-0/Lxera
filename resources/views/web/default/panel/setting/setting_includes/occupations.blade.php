            ...
            @if(!empty($category->subCategories) and count($category->subCategories))
                @foreach($category->subCategories as $subCategory)
                    <div class="checkbox-button mr-15 mt-10">
                        ...
                        <label class="font-14 text-dark-blue" for="checkbox{{ $subCategory->id }}">{{ $subCategory->title }}</label>
                    </div>
                @endforeach
            @else
                <div class="checkbox-button mr-15 mt-10">
                    ...
                    <label class="font-14 text-dark-blue" for="checkbox{{ $category->id }}">{{ $category->title }}</label>
                </div>
            @endif
        @endforeach
    </div>
