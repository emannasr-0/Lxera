...
@section('content')
    <section class=" js-font-resize maintenance-section mt-25 mb-50 position-relative">
        <div class=" js-font-resize container">
            <div class=" js-font-resize d-flex-center flex-column">
                @if(!empty($maintenanceSettings['image']))
                    <div class=" js-font-resize maintenance-image">
                        <img src="{{ $maintenanceSettings['image'] }}" alt="{{ $maintenanceSettings['title'] }}" class=" js-font-resize img-cover">
                    </div>
                @endif

                @if(!empty($maintenanceSettings['title']))
                    <h1 class=" js-font-resize font-36 font-weight-bold mt-10">{{ $maintenanceSettings['title'] }}</h1>
                @endif

                @if(!empty($maintenanceSettings['description']))
                    <p class=" js-font-resize font-14 font-weight-500 text-gray mt-15">{!! nl2br($maintenanceSettings['description']) !!}</p>
                @endif

                @if(!empty($remainingTimes))
                    <div id="maintenanceCountDown" class=" js-font-resize d-flex time-counter-down mt-15"
                ...
                @if(!empty($maintenanceSettings['maintenance_button']) and !empty($maintenanceSettings['maintenance_button']['title']) and !empty($maintenanceSettings['maintenance_button']['link']))
                    <a href="{{ $maintenanceSettings['maintenance_button']['link'] }}" class=" js-font-resize btn btn-primary mt-20">{{ $maintenanceSettings['maintenance_button']['title'] }}</a>
                @endif
            </div>
        </div>
    </section>
