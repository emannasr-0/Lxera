...
@section('content')
    ...
    <div class=" js-font-resize container pt-50 mt-10">
        ...
        {{--   @if ($installment->needToVerify()) --}}
        {{--  <h1 class=" js-font-resize font-36">مراجعة وتأكيد الأقساط </h1> --}}
        {{-- {{ trans('update.verify_your_installments') }} --}}
        {{-- <p class=" js-font-resize mt-10 font-16 text-gray">{{ trans('update.verify_your_installments_hint') }}</p> --}}

        {{--   @else --}}
        {{--  <h1 class=" js-font-resize font-36">مراجعة وتأكيد الأقساط </h1 > --}}
        {{-- {{ trans('update.verify_your_installments2') }} --}}
        {{-- <p class=" js-font-resize mt-10 font-16 text-gray">{{ trans('update.verify_your_installments_hint2') }}</p> --}}

        {{--  @endif --}}
        {{-- </div> --}}

        <div
            class=" js-font-resize become-instructor-packages d-flex align-items-center flex-column flex-lg-row mt-50 shadow border rounded-lg p-15 p-lg-25">
            ...
            <div class=" js-font-resize ml-lg-25 w-100 mt-20 mt-lg-0">
                <h2 class=" js-font-resize font-24 font-weight-bold text-pink">{{ trans('panel.overview_of_installment') }}</h2>
                {{-- <h2 class=" js-font-resize font-24 font-weight-bold text-light">{{ trans('update.installment_overview') }}</h2> --}}

                @if ($itemType == 'course')
                    <a href="{{ $item->getUrl() }}" target="_blank"
                        class=" js-font-resize font-14 font-weight-500 text-dark">Diploma in {{ $item->title }}</a>
                @else
                    <div class=" js-font-resize font-14 font-weight-500 text-dark">Diploma in {{ $item->title }}</div>
                @endif

                <div class=" js-font-resize d-flex flex-wrap align-items-center justify-content-around w-100">
                    ...
                    {{-- <div class=" js-font-resize d-flex align-items-center mt-20">
                        ...
                        <span class=" js-font-resize font-14 text-light ml-5">{{ !empty($installment->upfront) ? handlePrice($installment->getUpfront($itemPrice)).' '. trans('update.upfront') : trans('update.no_upfront') }}</span>
                    </div> --}}

                    <div class=" js-font-resize col-12 p-0 d-flex align-items-center mt-20">
                        <svg width="20" height="20" viewBox="0 0 24 21" fill="none"
                            ...
                            <g clip-path="url(#clip0_14_166)">
                                <path
                                    ...
                                    fill="#c14b93" class=" js-font-resize  cls-3 "></path>
                                <path
                        ...
                        <span class=" js-font-resize font-14 text-dark ml-5">{{ $installment->steps_count + 1 }}
...
<rect width="24" height="21" fill="white"></rect>
...
</svg>                        <span class=" js-font-resize font-14 text-light ml-5">{{ handlePrice($installment->totalPayments($itemPrice)) }} {{ trans('financial.total_amount') }}</span> --}}
                    {{-- </div> --}}

                    <!-- <div class=" js-font-resize d-flex align-items-center mt-20">
                        <i data-feather="calendar" width="20" height="20" class=" js-font-resize text-light"></i>

                        <span class=" js-font-resize font-14 text-light ml-5"> {{trans('panel.ends_on')}}
        ...
        <form action="/panel/bundles/purchase/{{ $installment->id }}" method="post">
            ...
            @if ($installment->request_uploads or $installment->needToVerify())
                <div class=" js-font-resize border rounded-lg p-15 mt-20">
                    @if ($installment->needToVerify())
                        <h3 class=" js-font-resize font-16 font-weight-bold text-dark-blue">{{ trans('update.verify_installments') }}</h3>

                        <div class=" js-font-resize font-16 text-gray mt-10">{!! nl2br($installment->verification_description) !!}</div>

                        {{-- Banner --}}
                        @if (!empty($installment->verification_banner))
                            <img src="{{ $installment->verification_banner }}" alt="{{ $installment->main_title }}"
                                class=" js-font-resize img-fluid mt-30">
                        @endif

                        {{-- Video --}}
                        @if (!empty($installment->verification_video))
                            <div class=" js-font-resize installment-video-card mt-50">
                                <video id="my-video" class=" js-font-resize video-js" controls preload="auto">
                    ...
                    @if ($installment->request_uploads)
                        <div class=" js-font-resize {{ $installment->needToVerify() ? 'mt-20' : '' }}">
                            <h4 class=" js-font-resize font-16 font-weight-bold text-dark-blue">{{ trans('update.attachments') }}</h4>
                            <p class=" js-font-resize mt-5 font-12 text-gray">
                                {{ trans('update.attach_your_documents_and_send_them_to_admin') }}</p>

                            @error('attachments')
                                <div class=" js-font-resize invalid-feedback d-block">{{ $message }}</div>
                            @enderror

                            <div class=" js-font-resize js-attachments">

                                <div class=" js-font-resize js-main-row row">
                                    <div class=" js-font-resize col-12 col-md-6 mt-15">
                                        <div class=" js-font-resize form-group mb-0">
                                            <label class=" js-font-resize font-14 text-dark-blue">{{ trans('public.title') }}</label>
                                            <input type="text" name="attachments[record][title]" class=" js-font-resize form-control">
                                        </div>
                                    </div>
                                    ...
                                    <div class=" js-font-resize col-12 col-md-6 mt-15">
                                        <div class=" js-font-resize form-group">
                                            <label
                                                class=" js-font-resize font-14 text-dark-blue">{{ trans('update.attach_a_file_optional') }}</label>

                                            <div class=" js-font-resize input-group">
                                                <div class=" js-font-resize input-group-prepend">
                                                    <button type="button" class=" js-font-resize input-group-text panel-file-manager"
                                                ...
                                                <input type="text" name="attachments[record][file]" id="file_record"
                                                    class=" js-font-resize form-control rounded-0" />

                                                <button type="button"
                                                    class=" js-font-resize js-add-btn btn btn-primary h-40px btn-sm installment-verify-attachment-add-btn">
            ...
            <div class=" js-font-resize border rounded-lg p-35 mt-30 shadow">
                <h3 class=" js-font-resize font-16 font-weight-bold text-pink">{{trans('panel.terms_and_rules_of_installment')}} </h3>
                <br>
                {{--   <div class=" js-font-resize font-16 text-pink">{!! nl2br(getInstallmentsTermsSettings('terms_description')) !!}</div> --}}
                <div class=" js-font-resize font-16 text-dark">
                ...
                <div class=" js-font-resize mt-10 border bg-secondary-acadima p-15 rounded-sm ">
                    <h4 class=" js-font-resize font-14 text-pink font-weight-bold">{{trans('panel.important')}}</h4>
                    <p class=" js-font-resize mt-5 font-14 text-dark">{{trans('panel.agreement')}}
                    </p>
                </div>
            </div>
            ...
            <div class=" js-font-resize d-flex align-items-center justify-content-between border-top pt-10 mt-20">
                <a href="{{ url()->previous() }}" class=" js-font-resize btn btn-sm btn-gray200">{{ trans('update.back') }}</a>

                <button type="submit" class=" js-font-resize btn btn-sm btn-primary">
                    @if ($installment->needToVerify())
