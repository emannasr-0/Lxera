    ...
    @if(!empty($user))
        <div class=" js-font-resize text-center">
            ...
            <p class=" js-font-resize font-14 font-weight-500 text-gray mt-30">
                <span class=" js-font-resize d-block">{{ trans('update.this_course_requires_n_points',['points' => $bundle->points]) }}</span>
        ...
        <div class=" js-font-resize d-flex align-items-center mt-25">
            <a href="{{ ($user->getRewardPoints() >= $bundle->points) ? '/bundles/'. $bundle->slug .'/points/apply' : '#' }}" class=" js-font-resize btn btn-sm flex-grow-1 {{ ($user->getRewardPoints() >= $bundle->points) ? 'btn-primary js-buy-course-with-point' : 'bg-gray300 text-gray disabled' }}">{{ trans('update.buy') }}</a>
