...
<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class=" js-font-resize accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class=" js-font-resize d-flex align-items-center justify-content-between " role="tab" id="session_{{ !empty($session) ? $session->id :'record' }}">
        <div class=" js-font-resize d-flex align-items-center" href="#collapseSession{{ !empty($session) ? $session->id :'record' }}" aria-controls="collapseSession{{ !empty($session) ? $session->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            <div class=" js-font-resize font-weight-bold text-dark-blue d-block cursor-pointer">{{ !empty($session) ? $session->title : trans('public.add_new_sessions') }}</div>
        </div>
        ...
        <div class=" js-font-resize d-flex align-items-center">
            ...
            @if(!empty($session))
                <a href="{{ getAdminPanelUrl() }}/sessions/{{ $session->id }}/delete" class=" js-font-resize delete-action btn btn-sm btn-transparent text-gray">
    ...
    <div id="collapseSession{{ !empty($session) ? $session->id :'record' }}" aria-labelledby="session_{{ !empty($session) ? $session->id :'record' }}" class=" js-font-resize  collapse @if(empty($session)) show @endif" role="tabpanel">
        <div class=" js-font-resize panel-collapse text-gray">
            <div class=" js-font-resize js-content-form session-form" data-action="{{ getAdminPanelUrl() }}/sessions/{{ !empty($session) ? $session->id . '/update' : 'store' }}">
                ...
                <div class=" js-font-resize row">
                    <div class=" js-font-resize col-12 col-lg-6">
                        ...
                        <div class=" js-font-resize form-group js-api-secret {{ (!empty($session) and in_array($session->session_api, ['zoom', 'agora', 'jitsi'])) ? 'd-none' :'' }}">
                            ...
                            <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][api_secret]" class=" js-font-resize js-ajax-api_secret form-control" value="{{ !empty($session) ? $session->api_secret : '' }}" {{ (!empty($session) and $session->session_api != 'local') ? 'disabled' :'' }}/>
                            <div class=" js-font-resize invalid-feedback"></div>
                        </div>
                        ...
                        <div class=" js-font-resize form-group js-moderator-secret {{ (empty($session) or $session->session_api != 'big_blue_button') ? 'd-none' :'' }}">
                            ...
                            <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][moderator_secret]" class=" js-font-resize js-ajax-moderator_secret form-control" value="{{ !empty($session) ? $session->moderator_secret : '' }}" {{ (!empty($session) and $session->session_api == 'big_blue_button') ? 'disabled' :'' }}/>
                            <div class=" js-font-resize invalid-feedback"></div>
                        </div>
                        ...
                        @if(!empty($session))
                            <div class=" js-font-resize form-group">
                                ...
                                <select name="ajax[{{ !empty($session) ? $session->id : 'new' }}][chapter_id]" class=" js-font-resize js-ajax-chapter_id form-control">
                        ...
                        <div class=" js-font-resize form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][title]" class=" js-font-resize js-ajax-title form-control" value="{{ !empty($session) ? $session->title : '' }}" placeholder="{{ trans('forms.maximum_255_characters') }}"/>
                            <div class=" js-font-resize invalid-feedback"></div>
                        </div>
                            ...
                            <div class=" js-font-resize input-group">
                                <div class=" js-font-resize input-group-prepend">
                                    <span class=" js-font-resize input-group-text" id="dateRangeLabel">
                                        <i data-feather="calendar" width="18" height="18" class=" js-font-resize "></i>
                                    </span>
                                </div>
                                <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][date]" class=" js-font-resize js-ajax-date form-control datetimepicker" value="{{ !empty($session) ? dateTimeFormat($session->date, 'Y-m-d H:i', false) : '' }}" aria-describedby="dateRangeLabel" {{ (!empty($session) and $session->session_api != 'local') ? 'disabled' :'' }} autocomplete="off"/>
                                <div class=" js-font-resize invalid-feedback"></div>
                            </div>
                        </div>
                        ...
                        <div class=" js-font-resize form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][duration]" class=" js-font-resize js-ajax-duration form-control" value="{{ !empty($session) ? $session->duration : '' }}" {{ (!empty($session) and $session->session_api != 'local') ? 'disabled' :'' }}/>
                            <div class=" js-font-resize invalid-feedback"></div>
                        </div>
                        ...
                        <div class=" js-font-resize form-group js-local-link {{ (!empty($session) and in_array($session->session_api, ['agora', 'jitsi'])) ? 'd-none' : '' }}">
                            ...
                            <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][link]" class=" js-font-resize js-ajax-link form-control" value="{{ !empty($session) ? $session->getJoinLink() : '' }}" {{ (!empty($session) and $session->session_api != 'local') ? 'disabled' :'' }}/>
                            <div class=" js-font-resize invalid-feedback"></div>
                        </div>
                        ...
                        @if(!empty(getFeaturesSettings('extra_time_to_join_status')) and getFeaturesSettings('extra_time_to_join_status'))
                            <div class=" js-font-resize form-group">
                                ...
                                <input type="text" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][extra_time_to_join]" value="{{ (!empty($session) and $session->extra_time_to_join) ? $session->extra_time_to_join : getFeaturesSettings('extra_time_to_join_default_value') }}" class=" js-font-resize js-ajax-extra_time_to_join form-control" placeholder=""/>
                                <div class=" js-font-resize invalid-feedback"></div>
                            </div>
                        @elseif(!empty(getFeaturesSettings('extra_time_to_join_default_value')))
                            <input type="hidden" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][extra_time_to_join]" value="{{ (!empty($session) and $session->extra_time_to_join) ? $session->extra_time_to_join : getFeaturesSettings('extra_time_to_join_default_value') }}" class=" js-font-resize js-ajax-extra_time_to_join form-control" placeholder=""/>
                        @endif
                        ...
                        <div class=" js-font-resize form-group mt-20">
                            <div class=" js-font-resize d-flex align-items-center justify-content-between">
                                ...
                                <div class=" js-font-resize custom-control custom-switch">
                                    <input type="checkbox" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][status]" class=" js-font-resize custom-control-input" id="sessionStatusSwitch{{ !empty($session) ? $session->id : '_record' }}" {{ (empty($session) or $session->status == \App\Models\Session::$Active) ? 'checked' : ''  }}>
                            ...
                            @if(getFeaturesSettings('agora_chat'))
                                <div class=" js-font-resize form-group mt-20">
                                    <div class=" js-font-resize d-flex align-items-center justify-content-between">
                                        ...
                                        <div class=" js-font-resize custom-control custom-switch">
                                            <input type="checkbox" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][agora_chat]" class=" js-font-resize custom-control-input" id="sessionAgoraChatSwitch{{ !empty($session) ? $session->id : '_record' }}" {{ (!empty($session) and !empty($session->agora_settings) and $session->agora_settings->chat) ? 'checked' : ''  }}>
                            ...
                            {{--
                                                        <div class=" js-font-resize form-group mt-20">
                                                            <div class=" js-font-resize d-flex align-items-center justify-content-between">
                                                                ...
                                                                <div class=" js-font-resize custom-control custom-switch">
                                                                    <input type="checkbox" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][agora_record]" class=" js-font-resize custom-control-input" id="sessionAgoraRecordSwitch{{ !empty($session) ? $session->id : '_record' }}" {{ (!empty($session) and !empty($session->agora_settings) and $session->agora_settings->record) ? 'checked' : ''  }}>
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            <div class=" js-font-resize form-group mt-20">
                                <div class=" js-font-resize d-flex align-items-center justify-content-between">
                                    <label class=" js-font-resize cursor-pointer input-label" for="SequenceContentSessionSwitch{{ !empty($session) ? $session->id : '_record' }}">{{ trans('update.sequence_content') }}</label>
                                    <div class=" js-font-resize custom-control custom-switch">
                                        <input type="checkbox" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][sequence_content]" class=" js-font-resize js-sequence-content-switch custom-control-input" id="SequenceContentSessionSwitch{{ !empty($session) ? $session->id : '_record' }}" {{ (!empty($session) and ($session->check_previous_parts or !empty($session->access_after_day))) ? 'checked' : ''  }}>
                                        <label class=" js-font-resize custom-control-label" for="SequenceContentSessionSwitch{{ !empty($session) ? $session->id : '_record' }}"></label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class=" js-font-resize js-sequence-content-inputs pl-5 {{ (!empty($session) and ($session->check_previous_parts or !empty($session->access_after_day))) ? '' : 'd-none' }}">
                                <div class=" js-font-resize form-group">
                                    <div class=" js-font-resize d-flex align-items-center justify-content-between">
                                        <label class=" js-font-resize cursor-pointer input-label" for="checkPreviousPartsSessionSwitch{{ !empty($session) ? $session->id : '_record' }}">{{ trans('update.check_previous_parts') }}</label>
                                        <div class=" js-font-resize custom-control custom-switch">
                                            <input type="checkbox" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][check_previous_parts]" class=" js-font-resize custom-control-input" id="checkPreviousPartsSessionSwitch{{ !empty($session) ? $session->id : '_record' }}" {{ (empty($session) or $session->check_previous_parts) ? 'checked' : ''  }}>
                                            <label class=" js-font-resize custom-control-label" for="checkPreviousPartsSessionSwitch{{ !empty($session) ? $session->id : '_record' }}"></label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class=" js-font-resize form-group">
                                    ...
                                    <input type="number" name="ajax[{{ !empty($session) ? $session->id : 'new' }}][access_after_day]" value="{{ (!empty($session)) ? $session->access_after_day : '' }}" class=" js-font-resize js-ajax-access_after_day form-control" placeholder="{{ trans('update.access_after_day_placeholder') }}"/>
                                    <div class=" js-font-resize invalid-feedback"></div>
                                </div>
                            </div>
                        @endif
