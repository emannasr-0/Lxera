...
@section('content')
    <section class=" js-font-resize section">
        <div class=" js-font-resize section-header">
            ...
            <div class=" js-font-resize section-header-breadcrumb">
                ...
                <div class=" js-font-resize breadcrumb-item"><a>{{ $pageTitle }}</a></div>
            </div>
        </div>
    </section>
    ...
    <div class=" js-font-resize card">
        <div class=" js-font-resize card-header">
            @can('admin_webinar_notification_to_students')
                <a href="{{ getAdminPanelUrl() }}/webinars/{{ $webinar->id }}/sendNotification" class=" js-font-resize btn btn-primary mr-2">{{ trans('notification.send_notification') }}</a>
            @endcan

            @can('admin_enrollment_add_student_to_items')
                <button type="button" id="addStudentToCourse" class=" js-font-resize btn btn-primary mr-2">{{ trans('update.add_student_to_course') }}</button>
            @endcan
            ...
            <div class=" js-font-resize table-responsive text-center">
                <table class=" js-font-resize table table-striped font-14">
                    ...
                    @foreach($students as $student)
                        ...
                        <tr>
                            <td class=" js-font-resize text-left">{{ $student->id ?? '-' }}</td>
                            <td class=" js-font-resize text-left">
                                <div class=" js-font-resize d-flex align-items-center">
                                    ...
                                    <div class=" js-font-resize media-body ml-1">
                                        <div class=" js-font-resize mt-0 mb-1 font-weight-bold">{{ $student->full_name }}</div>

                                        @if($student->mobile)
                                            <div class=" js-font-resize text-primary text-small font-600-bold">{{ $student->mobile }}</div>
                                        @endif

                                        @if($student->email)
                                            <div class=" js-font-resize text-primary text-small font-600-bold">{{ $student->email }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                @if(empty($student->id))
                                    ...
                                    <div class=" js-font-resize mt-0 mb-1 font-weight-bold text-warning">{{ trans('update.unregistered') }}</div>
                                @elseif(!empty($webinar->access_days) and !$webinar->checkHasExpiredAccessDays($student->purchase_date, $student->gift_id))
                                    <div class=" js-font-resize mt-0 mb-1 font-weight-bold text-warning">{{ trans('panel.expired') }}</div>
                                @elseif(!$student->access_to_purchased_item)
                                    <div class=" js-font-resize mt-0 mb-1 font-weight-bold text-danger">{{ trans('update.access_blocked') }}</div>
                                @else
                                    <div class=" js-font-resize mt-0 mb-1 font-weight-bold text-success">{{ trans('admin/main.active') }}</div>
                                @endif
                            </td>
                            ...
                            <td class=" js-font-resize text-center mb-2" width="120">
                                @if(!empty($student->id))
                                    ...
                                    @can('admin_users_impersonate')
                                        <a href="{{ getAdminPanelUrl() }}/users/{{ $student->id }}/impersonate" target="_blank" class=" js-font-resize btn-transparent  text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('admin/main.login') }}">
                                            <i class=" js-font-resize fa fa-user-shield"></i>
                                        </a>
                                    @endcan

                                    @can('admin_users_edit')
                                        <a href="{{ getAdminPanelUrl() }}/users/{{ $student->id }}/edit" class=" js-font-resize btn-transparent  text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('admin/main.edit') }}">
    ...
    <div id="addStudentToCourseModal" class=" js-font-resize d-none">
        <h3 class=" js-font-resize section-title after-line">{{ trans('update.add_student_to_course') }}</h3>
        <div class=" js-font-resize mt-25">
            <form action="{{ getAdminPanelUrl() }}/enrollments/store" method="post">
                ...
                <div class=" js-font-resize d-flex align-items-center justify-content-end mt-3">
                    <button type="button" class=" js-font-resize js-save-manual-add btn btn-sm btn-primary">{{ trans('public.save') }}</button>
