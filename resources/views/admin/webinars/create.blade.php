                                                ...
                                                <select name="student_id[]" id="student_id" class="form-control select2" multiple data-placeholder="قم بتحديد الطلاب">
                                                    ...
                                                    @foreach ($students as $student)
                                                        <option value="{{ $student->id }}"
                                                            @if(in_array($student->id , old('student_id', [])) || (!empty($webinar)&& $webinar->studentsExcluded->contains($student->id)))
                                                                selected
                                                            @endif>
                                                            {{ $student->registeredUser->email ?? '' }} - {{ $student->ar_name ?? '' }} ({{ $student->registeredUser->user_code ?? '' }})
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
