    ...
    <div class="col-12 col-md-5">
        ...
        <div class="form-group mt-15 ">
            ...
            <select name="target_type"
                class="js-target-types-input custom-select @error('target_types')  is-invalid @enderror">
                @foreach (\App\Models\Installment::$targetTypes as $type)
                    <option value="{{ $type }}" @if (old('target_types')== $type || (!empty($installment) and $installment->target_type == $type)) selected @endif>
                        {{ trans('update.target_types_' . $type) }}</option>
                @endforeach
            </select>
            ...
            <select name="target" class="js-target-input custom-select  @error('target')  is-invalid @enderror">
                ...
                @foreach ($targets as $targetKey => $targetItems)
                    @foreach ($targetItems as $target)
                        <option value="{{ $target }}"
                            class="js-target-option js-target-option-{{ $targetKey }} {{ (!empty($installment) and $installment->target_type == $targetKey) ? '' : 'd-none' }}"
                            @if ((!empty($installment) and $installment->target == $target) || old('target') == $target) selected @endif>{{ trans('update.' . $target) }}</option>
                    @endforeach
                @endforeach
            </select>
        ...
        <div
            class="form-group js-specific-categories-field {{ (!empty($installment) and $installment->target == 'specific_categories') ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('update.specific_categories') }}</label>

            <select name="category_ids[]" id="categories" class="custom-select select2" multiple
        ...
        <div
            class="form-group js-specific-instructors-field {{ (!empty($installment) and $installment->target == 'specific_instructors') ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('update.specific_instructors') }}</label>

            <select name="instructor_ids[]" multiple="multiple" data-search-option="just_teacher_role"
                class="form-control search-user-select2" data-placeholder="{{ trans('public.search_instructors') }}">
        ...
        <div
            class="form-group js-specific-sellers-field {{ (!empty($installment) and $installment->target == 'specific_sellers') ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('update.specific_sellers') }}</label>

            <select name="seller_ids[]" multiple="multiple" data-search-option="except_user"
                class="form-control search-user-select2" data-placeholder="{{ trans('public.search_instructors') }}">
        ...
        <div
            class="form-group js-specific-courses-field {{ (!empty($installment) and $installment->target == 'specific_courses') ? '' : 'd-none' }}">
        ...
        <div
            class="form-group js-specific-products-field {{ (!empty($installment) and $installment->target == 'specific_products') ? '' : 'd-none' }}">
            ...
            <select name="bundle_ids[]" multiple="multiple" class="form-control search-bundle-select2 " data-placeholder="Search bundles">
                ...
                @if (!empty($installment) and count($installment->bundles))
                    @foreach ($installment->bundles as $bundle)
                        <option value="{{ $bundle->id }}" selected>{{ $bundle->title }} ({{ $bundle?->batch?->title }} )</option>
                    @endforeach
                @endif
            </select>
        ...
        <div
            class="form-group  js-specific-bundles-field {{ (!empty($installment) and $installment->target == 'specific_bundles') ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('update.specific_bundles') }}</label>
            <select name="bundle_ids[]" multiple="multiple" class="form-control" id="specific_bundles"
                data-placeholder="Search bundles">

                @if (!empty($bundles))
                    @foreach ($bundles as $bundle)
                        <option value="{{ $bundle->id }}" @if (in_array($bundle->id, old('bundle_ids', [])) ||
                                (!empty($installment) && $installment->bundles->contains($bundle->id))) selected @endif>
                            {{ $bundle->title }}
                            ({{ $bundle?->batch?->title }} -
                            @if ($bundle->status == 'active')
                                <span class='text-success'>مفتوح</span>
                            @else
                                <span class='text-danager'>مغلق</span>
                            @endif)
                        </option>
                    @endforeach
                @endif
        ...
        @php
            $selectedSubscriptionPackagesIds = !empty($installment)
                ? $installment->subscribes->pluck('id')->toArray()
                : [];
        @endphp
        {{-- Subscription Specific Package --}}
        <div
            class="form-group js-subscription-specific-packages-field {{ (!empty($installment) and $installment->target_type == 'subscription_packages' and $installment->target == 'specific_packages') ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('update.specific_packages') }}</label>
            <select name="subscribe_ids[]" multiple="multiple" class="form-control select2"
                data-placeholder="{{ trans('update.select_packages') }}">

                @if (!empty($subscriptionPackages) and $subscriptionPackages->count() > 0)
                    @foreach ($subscriptionPackages as $subscriptionPackage)
                        <option value="{{ $subscriptionPackage->id }}"
                            {{ in_array($subscriptionPackage->id, $selectedSubscriptionPackagesIds) ? 'selected' : '' }}>
                            {{ $subscriptionPackage->title }}</option>
                    @endforeach
                @endif
            </select>
        ...
        {{--
            <div class="form-group mt-15">
                ...
                <select id="study_classes" class="custom-select @error('batch_id') is-invalid @enderror" name="batch_id"
                    required>
        ...
        @php
            $selectedRegistrationPackagesIds = !empty($installment)
                ? $installment->registrationPackages->pluck('id')->toArray()
                : [];
        @endphp
        {{-- Registration Specific Package --}}
        <div
            class="form-group js-registration-specific-packages-field {{ (!empty($installment) and $installment->target_type == 'registration_packages' and $installment->target == 'specific_packages') ? '' : 'd-none' }}">
            <label class="input-label">{{ trans('update.specific_packages') }}</label>
            <select name="registration_package_ids[]" multiple="multiple" class="form-control select2"
                data-placeholder="{{ trans('update.select_packages') }}">

                @if (!empty($registrationPackages) and $registrationPackages->count() > 0)
                    @foreach ($registrationPackages as $registrationPackage)
                        <option value="{{ $registrationPackage->id }}"
                            {{ in_array($registrationPackage->id, $selectedRegistrationPackagesIds) ? 'selected' : '' }}>
                            {{ $registrationPackage->title }} ({{ $registrationPackage->role }})</option>
                    @endforeach
                @endif
            </select>
