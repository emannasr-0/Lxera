@foreach(\App\Models\WebinarExtraDescription::$types as $extraDescriptionType)
    <section class=" js-font-resize mt-30">
        <div class=" js-font-resize d-flex justify-content-between align-items-center">
            <h2 class=" js-font-resize section-title after-line">{{ trans('update.'.$extraDescriptionType) }}</h2>
            <button id="add_new_{{ $extraDescriptionType }}" type="button" class=" js-font-resize btn btn-primary btn-sm mt-3">{{ trans('update.add_'.$extraDescriptionType) }}</button>
        </div>
                            ...
                            @foreach($webinarExtraDescriptionValues as $extraDescription)
                                <tr>
                                    ...
                                    <td class=" js-font-resize text-right">
                                        <button type="button" data-item-id="{{ $extraDescription->id }}" data-webinar-id="{{ !empty($upcomingCourse) ? $upcomingCourse->id : '' }}" class=" js-font-resize edit-extraDescription btn-transparent text-primary mt-1" data-toggle="tooltip" data-placement="top" title="{{ trans('admin/main.edit') }}">
