    ...
    <section class=" js-font-resize section">
        <div class=" js-font-resize section-header">
            ...
            <div class=" js-font-resize section-header-breadcrumb">
                ...
                <div class=" js-font-resize breadcrumb-item">{{ trans('update.upcoming_courses') }}</div>
            </div>
        </div>
                ...
                <div class=" js-font-resize col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class=" js-font-resize card card-statistic-1">
                        <div class=" js-font-resize card-icon bg-primary">
                            <i class=" js-font-resize fas fa-video"></i>
                        </div>
                ...
                <div class=" js-font-resize col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class=" js-font-resize card card-statistic-1">
                        <div class=" js-font-resize card-icon bg-warning">
                            <i class=" js-font-resize fas fa-calendar-check"></i>
                        </div>
                ...
                <div class=" js-font-resize col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class=" js-font-resize card card-statistic-1">
                        <div class=" js-font-resize card-icon bg-info">
                            <i class=" js-font-resize fas fa-clock"></i>
                        </div>
                ...
                <div class=" js-font-resize col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class=" js-font-resize card card-statistic-1">
                        <div class=" js-font-resize card-icon bg-success">
                            <i class=" js-font-resize fas fa-users"></i></div>
                        <div class=" js-font-resize card-wrap">
                            <div class=" js-font-resize card-header">
                ...
                <div class=" js-font-resize card-body">
                    <form method="get" class=" js-font-resize mb-0">
                        ...
                        <div class=" js-font-resize row">
                            ...
                            <div class=" js-font-resize col-md-3">
                                <div class=" js-font-resize form-group">
                                    <label class=" js-font-resize input-label">{{trans('update.release_date')}}</label>
                        ...
                        <div class=" js-font-resize card-header">
                            @can('admin_upcoming_courses_export_excel')
                                <div class=" js-font-resize text-right">
                                    <a href="{{ getAdminPanelUrl('/upcoming_courses/excel?'. http_build_query(request()->all())) }}" class=" js-font-resize btn btn-primary">{{ trans('admin/main.export_xls') }}</a>
                                </div>
                            @endcan
                        </div>
                                    ...
                                    @foreach($upcomingCourses as $upcomingCourse)
                                        <tr class=" js-font-resize text-center">
                                            ...
                                            <td width="18%" class=" js-font-resize text-left">
                                                <a class=" js-font-resize text-primary mt-0 mb-1 font-weight-bold" href="{{ $upcomingCourse->getUrl() }}">{{ $upcomingCourse->title }}</a>
                                                @if(!empty($upcomingCourse->category->title))
                                                    <div class=" js-font-resize text-small">{{ $upcomingCourse->category->title }}</div>
                                                @else
                                                    <div class=" js-font-resize text-small text-warning">{{trans('admin/main.no_category')}}</div>
                                                @endif
                                            </td>

                                            <td class=" js-font-resize text-left">{{ $upcomingCourse->teacher->full_name }}</td>

                                            <td class=" js-font-resize ">{{ trans('admin/main.'.$upcomingCourse->type) }}</td>
                                            ...
                                            <td class=" js-font-resize font-12">
                                                <a href="{{ getAdminPanelUrl('/upcoming_courses/'. $upcomingCourse->id .'/followers') }}" target="_blank" class=" js-font-resize ">{{ $upcomingCourse->followers_count }}</a>
                                            </td>

                                            <td class=" js-font-resize font-12">{{ dateTimeFormat($upcomingCourse->publish_date, 'Y M j | H:i') }}</td>

                                            <td class=" js-font-resize font-12">{{ dateTimeFormat($upcomingCourse->created_at, 'Y M j | H:i') }}</td>

                                            <td>
                                                @if(!empty($upcomingCourse->webinar_id))
                                                    <div class=" js-font-resize text-success font-600-bold">{{ trans('update.released') }}</div>
                                                @else
                                                    @switch($upcomingCourse->status)
                                                        @case(\App\Models\Webinar::$active)
                                                            <div class=" js-font-resize text-primary">{{ trans('admin/main.published') }}</div>
                                                            @break
                                                ...
                                                <div class=" js-font-resize btn-group dropdown table-actions">
                                                    ...
                                                    <div class=" js-font-resize dropdown-menu text-left webinars-lists-dropdown">
                                                        ...
                                                        @can('admin_upcoming_courses_edit')
                                                            @if($upcomingCourse->status != \App\Models\Webinar::$active)
                                                                @include('admin.includes.delete_button',[
                                                                    ...
                                                                    'btnText' => '<i class=" js-font-resize fa fa-times"></i><span class=" js-font-resize ml-2">'. trans("admin/main.approve") .'</span>'
                                                                    ])
                                                            @endif
                                                        ...
                                                        @can('admin_upcoming_courses_followers')
                                                            <a href="{{ getAdminPanelUrl() }}/upcoming_courses/{{ $upcomingCourse->id }}/followers" target="_blank" class=" js-font-resize d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('update.followers') }}">
                                                                <i class=" js-font-resize fa fa-users"></i>
                                                                <span class=" js-font-resize ml-2">{{ trans('update.followers') }}</span>
                                                            </a>
                                                        @endcan

                                                        @can('admin_upcoming_courses_edit')
                                                            <a href="{{ getAdminPanelUrl('/upcoming_courses/'. $upcomingCourse->id .'/edit') }}" target="_blank" class=" js-font-resize d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('admin/main.edit') }}">
