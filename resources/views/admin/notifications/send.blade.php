        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="post"
                        action="{{ getAdminPanelUrl() }}/notifications/{{ !empty($notification) ? $notification->id . '/update' : 'store' }}"
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title"
                                        ...
                                        value="{{ !empty($notification) ? $notification->title : old('title') }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" id="typeSelect"
                                        ...
                                        @foreach (\App\Models\Notification::$notificationsType as $type)
                                            <option value="{{ $type }}"
                                                @if (!empty($notification) and $notification->type == $type) selected @endif>
                                                {{ trans('admin/main.notification_' . $type) }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback">
                                        @error('type')
                                    ...
                                    <div class="text-muted text-small mt-1">{{ trans('admin/main.new_notification_hint') }}
                                    </div>
                                </div>
                                ...
                                <div class="form-group @if (!$errors->has('user_id') and (empty($notification) or empty($notification->user))) d-none @endif" id="userSelect">
                                    ...
                                    <select name="user_id"
                                        class="form-control search-user-select2 @error('user_id') is-invalid @enderror"
                                        data-placeholder="{{ trans('public.search_user') }}">

                                        @if (!empty($notification) and !empty($notification->user))
                                            <option value="{{ $notification->user->id }}" selected>
                                                {{ $notification->user->full_name }}</option>
                                        @endif
                                    </select>
                                    <div class="invalid-feedback">
                                        @error('user_id')
                                ...
                                <div class="form-group @if (!$errors->has('webinar_id') and (empty($notification) or empty($notification->webinar))) d-none @endif" id="webinarSelect">
                                    ...
                                    <select name="webinar_id"
                                        class="form-control search-webinar-select2 @error('webinar_id') is-invalid @enderror"
                                        data-placeholder="{{ trans('admin/main.search_webinar') }}">

                                        @if (!empty($notification) and !empty($notification->webinar))
                                            <option value="{{ $notification->webinar->id }}" selected>
                                                {{ $notification->webinar->title }}</option>
                                        @endif
                                    </select>
                                    <div class="invalid-feedback">
                                        @error('webinar_id')
                                    ...
                                    <select name="group_id"
                                        ...
                                        @foreach ($userGroups as $userGroup)
                                            <option value="{{ $userGroup->id }}"
                                                ...
                                                @if ($userGroup->webinar_id)
                                                    {{ $userGroup->webinar->title }}
                                                @elseif ($userGroup->bundle_id)
                                                    {{ $userGroup->bundle->title }}
                                                @endif
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                <button class="btn btn-primary"
                                    type="submit">{{ trans('notification.send_notification') }}</button>
                            </div>
                        </div>
