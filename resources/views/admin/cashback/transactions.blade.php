                        ...
                        <div class=" js-font-resize row">
                            ...
                            <div class=" js-font-resize col-md-3">
                                <div class=" js-font-resize form-group">
                                    ...
                                    <select name="user_ids[]" multiple="multiple" class=" js-font-resize form-control search-user-select2"
                            ...
                            <div class=" js-font-resize col-md-3">
                                <div class=" js-font-resize form-group">
                                    <label class=" js-font-resize input-label">{{trans('admin/main.type')}}</label>
                                    <select name="target_type" class=" js-font-resize form-control populate">
                                        <option value="">{{trans('admin/main.all')}}</option>
                        ...
                        <div class=" js-font-resize card-header">
                            @can('admin_cashback_transactions')
                                <div class=" js-font-resize text-right">
                                    <a href="{{ getAdminPanelUrl('/cashback/transactions/excel?'. http_build_query(request()->all())) }}" class=" js-font-resize btn btn-primary">{{ trans('admin/main.export_xls') }}</a>
                                </div>
                            @endcan
                        </div>
                            ...
                            <div class=" js-font-resize table-responsive">
                                <table class=" js-font-resize table table-striped font-14 ">
                                    <tr>
                                        ...
                                        <th class=" js-font-resize ">{{trans('admin/main.description')}}</th>
                                    ...
                                    @foreach($transactions as $transaction)
                                        <tr class=" js-font-resize text-center">
                                                ...
                                                <div class=" js-font-resize d-flex align-items-center">
                                                    @if(!empty($transaction->user))
                                                        ...
                                                        <div class=" js-font-resize media-body ml-1">
                                                            <div class=" js-font-resize mt-0 mb-1 font-weight-bold">{{ $transaction->user->full_name }}</div>

                                                            @if($transaction->user->mobile)
                                                                <div class=" js-font-resize text-primary text-small font-600-bold">{{ $transaction->user->mobile }}</div>
                                                            @endif

                                                            @if($transaction->user->email)
                                                                <div class=" js-font-resize text-primary text-small font-600-bold">{{ $transaction->user->email }}</div>
                                                            @endif
                                                        </div>
                                                    @else
                                                        <span class=" js-font-resize fs-11">{{ trans('update.deleted_user') }}</span>
                                                    @endif
                                                </div>
                                            </td>
                                                ...
                                                <div class=" js-font-resize ">
                                                    @if(!empty($transaction->webinar_id))
                                                        <a href="{{ !empty($transaction->webinar) ? $transaction->webinar->getUrl() : '' }}"
                                                           target="_blank" class=" js-font-resize font-14">#{{ $transaction->webinar_id }}-{{ !empty($transaction->webinar) ? $transaction->webinar->title : '' }}</a>
                                                        <span class=" js-font-resize d-block font-12">{{ trans('update.target_types_courses') }}</span>
                                                    @elseif(!empty($transaction->bundle_id))
                                                        <a href="{{ !empty($transaction->bundle) ? $transaction->bundle->getUrl() : '' }}"
                                                           target="_blank" class=" js-font-resize font-14">#{{ $transaction->bundle_id }}-{{ !empty($transaction->bundle) ? $transaction->bundle->title : '' }}</a>
                                                        <span class=" js-font-resize d-block font-12">{{ trans('update.target_types_bundles') }}</span>
                                                    @elseif(!empty($transaction->product_id))
                                                        <a href="{{ !empty($transaction->product) ? $transaction->product->getUrl() : '' }}"
                                                           target="_blank" class=" js-font-resize font-14">#{{ $transaction->product_id }}-{{ !empty($transaction->product) ? $transaction->product->title : '' }}</a>
                                                        <span class=" js-font-resize d-block font-12">{{ trans('update.target_types_store_products') }}</span>
                                                    @elseif(!empty($transaction->meeting_time_id))
                                                        <div class=" js-font-resize font-14">#{{ $transaction->meeting_time_id }} {{ trans('admin/main.meeting') }}</div>
                                                        <span class=" js-font-resize d-block font-12">{{ trans('update.target_types_meetings') }}</span>
                                                    @elseif(!empty($transaction->subscribe_id))
                                                        <span class=" js-font-resize font-14">{{ trans('admin/main.purchased_subscribe') }}</span>
                                                        <span class=" js-font-resize d-block font-12">{{ trans('update.target_types_subscription_packages') }}</span>
                                                    @elseif(!empty($transaction->promotion_id))
                                                        <span class=" js-font-resize font-14">{{ trans('admin/main.purchased_promotion') }}</span>
                                                    @elseif(!empty($transaction->registration_package_id))
                                                        <span class=" js-font-resize font-14">{{ trans('update.purchased_registration_package') }}</span>
                                                        <span class=" js-font-resize d-block font-12">{{ trans('update.target_types_registration_packages') }}</span>
                                                    @else
                                                        ---
                                                    @endif
                                                </div>
                                            ...
                                            <td class=" js-font-resize font-12">{{ dateTimeFormat($transaction->created_at, 'j M Y') }}</td>

                                            <td class=" js-font-resize ">
                                                @if($transaction->system)
                                                    <span class=" js-font-resize font-12 text-danger">{{ trans('admin/main.refund') }}</span>
                                                @else
                                                    <span class=" js-font-resize font-12 text-success">{{ trans('update.successful') }}</span>
                                                @endif
                                            </td>
